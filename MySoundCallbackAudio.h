...
int MySoundCallbackAudio(char* buffer, int bufferSize, void* WXUNUSED(userData))
{
	int i, j, k;
	...
	for (j = 0; j < g_sound->n_midiDevices; j++)
	{
		...
		for(;;)
		{
			...
			if (msg.empty())
				break;
			MySoundCallbackMIDI(msg, j);
		}
	}

	if (!g_sound->b_active || !g_sound->samplers_count)
		mymemset(buffer, 0, bufferSize);
	if (!g_sound->b_active)
		return 0;
	...
	for (j = 0; j < organfile->NumberOfTremulants + 1 + organfile->NumberOfWindchestGroups; j++)
	{
		...
		else
		{
			...
			{
				register MyEnclosure* enclosure = organfile->enclosure + (organfile->windchest[j].enclosure[i]);
			...
			float f = d;
            mymemset(volume_buff, *(int*)&f, bufferSize);
		}

        mymemset(this_buff, j < organfile->NumberOfTremulants ? 0x800000 : 0, bufferSize);
		int *this_buff_end = this_buff + bufferSize;
		for (register MySampler* sampler = g_sound->windchests[j]; sampler; sampler = sampler->next)
		{
