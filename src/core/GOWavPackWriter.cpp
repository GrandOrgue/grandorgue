...
bool GOWavPackWriter::Init(unsigned channels, unsigned bitsPerSample,
                           unsigned bytesPerSample, unsigned sampleRate,
...
bool GOWavPackWriter::AddSampleData(GOBuffer<int32_t>& sampleData) {
  ...
  return WavpackPackSamples(
             m_Context, sampleData.get(),
             sampleData.GetCount() / WavpackGetNumChannels(m_Context)) != 0;
}
