...
GOFile* GOArchive::OpenFile(const wxString& name)
{
	for(unsigned i = 0; i < m_Entries.size(); i++)
		if (m_Entries[i].name == name)
		{
			return new GOArchiveEntryFile(this, m_Entries[i].name, m_Entries[i].offset, m_Entries[i].len);
		}
...
const wxString& GOArchive::GetArchiveID()
{
	return m_ID;
}
const wxString& GOArchive::GetPath()
{
	return m_Path;
}
