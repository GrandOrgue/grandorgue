...
GOArchiveIndex::GOArchiveIndex(const GOSettingDirectory& cachePath,
                               const wxString& path)
...
bool GOArchiveIndex::ReadContent(wxString& id,
...
bool GOArchiveIndex::WriteContent(const wxString& id,
...
bool GOArchiveIndex::ReadIndex(wxString& id,
                               std::vector<GOArchiveEntry>& entries) {
  ...
  if (magic != GRANDORGUE_INDEX_MAGIC ||
...
bool GOArchiveIndex::WriteIndex(const wxString& id,
                                const std::vector<GOArchiveEntry>& entries) {
  wxString name = GenerateIndexFilename();
