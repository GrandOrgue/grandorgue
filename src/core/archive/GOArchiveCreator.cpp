...
std::unique_ptr<GOFile> GOArchiveCreator::findPackageFile(const wxString& name)
{
...
bool GOArchiveCreator::storeFile(const wxString& name, const GOBuffer<uint8_t>& data)
{
...
bool GOArchiveCreator::CreatePackage(const wxString& path, const wxString title)
{
...
bool GOArchiveCreator::addOrganData(unsigned idx, GOFile* file)
{
	...
	m_organs[idx] = new GOOrgan(m_OrganPaths[idx], wxEmptyString, church_name, organ_builder, recording_details);
	return true;
}

bool GOArchiveCreator::compressData(const wxString& name, const wxString& ext, GOBuffer<uint8_t>& data)
{
