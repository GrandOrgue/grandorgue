...
GOArchiveManager::GOArchiveManager(GOOrganList& OrganList, const GOSettingDirectory& CacheDir) :
...
bool GOArchiveManager::ReadIndex(GOArchive* archive, bool InstallOrgans)
{
	...
	try
	{
		...
		for(unsigned i = 0; i < organ_count; i++)
		{
			...
			if (InstallOrgans)
				organs.push_back(GOOrgan(odf, id, ChurchName, OrganBuilder, RecordingDetails));
		}

		GOArchiveFile a(id, archive->GetPath(), package_name, depend, depend_titles);
...
wxString GOArchiveManager::InstallPackage(const wxString& path, const wxString& last_id)
{
