...
GOArchiveFile::GOArchiveFile(wxString id, wxString path, wxString name,
...
void GOArchiveFile::Save(GOConfigWriter& cfg, wxString group) {
  ...
  for (unsigned i = 0; i < m_Dependencies.size(); i++) {
    cfg.WriteString(group, wxString::Format("Dependency%03d", i + 1),
                    m_Dependencies[i]);
    cfg.WriteString(group, wxString::Format("DependencyTitle%03d", i + 1),
