...
bool GOConfigReader::Read(GOSettingType type, wxString group, wxString key,
                          bool required, wxString& value) {
  bool found = false;

  if (required) {
    if (group.length() >= 6 &&
        group.Mid(0, 6) == wxT("Setter"))  // Setter groups aren't required.
      required = false;
    else if (group.length() >= 5 && group.Mid(0, 5) == wxT("Panel")) {
      if (group.length() >= 14 &&
          group.Mid(8, 6) == wxT("Setter"))  // Setter groups aren't required.
        required = false;
  ...
  if (!found) {
    if (required) {
      wxString error;
      error.Printf(_("Missing required value section '%s' entry '%s'"),
...
wxString GOConfigReader::ReadString(GOSettingType type, wxString group,
                                    wxString key, bool required,
...
wxString GOConfigReader::ReadStringTrim(GOSettingType type, wxString group,
                                        wxString key, bool required,
                                        wxString defaultValue) {
  ...
  if (value.length() > 0 && value[value.length() - 1] == ' ') {
    if (m_Strict)
      wxLogWarning(_("Trailing whitespace at section '%s' entry '%s': %s"),
...
wxString GOConfigReader::ReadStringNotEmpty(GOSettingType type, wxString group,
                                            wxString key, bool required,
                                            wxString defaultValue) {
  wxString value = ReadString(type, group, key, required, defaultValue);

  if (value.Trim().length() == 0 && m_Strict) {
    wxLogWarning(_("Empty at section '%s' entry '%s'"), group.c_str(),
                 key.c_str());
  }
  return value;
}
...
bool GOConfigReader::ReadBoolean(GOSettingType type, wxString group,
                                 wxString key, bool required) {
...
bool GOConfigReader::ReadBoolean(GOSettingType type, wxString group,
  ...
  value.MakeUpper();
  wxLogWarning(_("Strange boolean value for section '%s' entry '%s': %s"),
               group.c_str(), key.c_str(), value.c_str());
  ...
  wxString error;
  error.Printf(_("Invalid boolean value at section '%s' entry '%s': %s"),
               group.c_str(), key.c_str(), value.c_str());
  throw error;
}

wxColour GOConfigReader::ReadColor(GOSettingType type, wxString group,
                                   wxString key, bool required) {
...
wxColour GOConfigReader::ReadColor(GOSettingType type, wxString group,
  ...
  wxString error;
  error.Printf(_("Invalid color at section '%s' entry '%s': %s"), group.c_str(),
               key.c_str(), value.c_str());
  throw error;
}

int GOConfigReader::ReadInteger(GOSettingType type, wxString group,
                                wxString key, int nmin, int nmax,
                                bool required) {
  return ReadInteger(type, group, key, nmin, nmax, required, nmin);
}

int GOConfigReader::ReadInteger(GOSettingType type, wxString group,
                                wxString key, int nmin, int nmax, bool required,
                                int defaultValue) {
  wxString value;
  if (!Read(type, group, key, required, value)) return defaultValue;
  ...
  if (!parseLong(retval, value)) {
    if (value.Length() && !::wxIsdigit(value[0]) && value[0] != wxT('+') &&
        value[0] != wxT('-') && value.CmpNoCase(wxT("none")) &&
        !value.IsEmpty()) {
      wxString error;
      error.Printf(_("Invalid integer value at section '%s' entry '%s': %s"),
                   group.c_str(), key.c_str(), value.c_str());
      throw error;
    }

    retval = wxAtoi(value);
    wxLogWarning(_("Invalid integer value at section '%s' entry '%s': %s"),
                 group.c_str(), key.c_str(), value.c_str());
  }
  ...
  wxString error;
  error.Printf(_("Out of range value at section '%s' entry '%s': %d"),
               group.c_str(), key.c_str(), retval);
  throw error;
}

int GOConfigReader::ReadLong(GOSettingType type, wxString group, wxString key,
                             int nmin, int nmax, bool required) {
...
double GOConfigReader::ReadFloat(GOSettingType type, wxString group,
                                 wxString key, double nmin, double nmax,
                                 bool required) {
...
double GOConfigReader::ReadFloat(GOSettingType type, wxString group,
                                 wxString key, double nmin, double nmax,
                                 bool required, double defaultValue) {
  wxString value;
  if (!Read(type, group, key, required, value)) return defaultValue;
  ...
  if (pos >= 0) {
    wxLogWarning(_("Number %s contains locale dependent floating point"),
                 value.c_str());
    value[pos] = wxT('.');
  }
  if (!parseCDouble(retval, value)) {
    wxString error;
    error.Printf(_("Invalid float value at section '%s' entry '%s': %s"),
                 group.c_str(), key.c_str(), value.c_str());
    throw error;
  }
  ...
  wxString error;
  error.Printf(_("Out of range value at section '%s' entry '%s': %f"),
               group.c_str(), key.c_str(), retval);
  throw error;
}

unsigned GOConfigReader::ReadSize(GOSettingType type, wxString group,
                                  wxString key, unsigned size_type,
                                  bool required) {
...
unsigned GOConfigReader::ReadSize(GOSettingType type, wxString group,
                                  wxString key, unsigned size_type,
                                  bool required, wxString defaultValue) {
  static const int sizes[2][4] = {{800, 1007, 1263, 1583},
                                  {500, 663, 855, 1095}};
  wxString value;
  ...
  wxString error;
  error.Printf(_("Invalid size at section '%s' entry '%s': %s"), group.c_str(),
               key.c_str(), value.c_str());
  throw error;
}

unsigned GOConfigReader::ReadFontSize(GOSettingType type, wxString group,
                                      wxString key, bool required) {
...
unsigned GOConfigReader::ReadFontSize(GOSettingType type, wxString group,
  ...
  wxString error;
  error.Printf(_("Invalid font size at section '%s' entry '%s': %s"),
               group.c_str(), key.c_str(), value.c_str());
  throw error;
}

int GOConfigReader::ReadEnum(GOSettingType type, wxString group, wxString key,
                             const struct IniFileEnumEntry* entry,
                             unsigned count, bool required, int defaultValue) {
  int defaultEntry = -1;
  wxString value;
  ...
  wxString error;
  error.Printf(_("Invalid enum value at section '%s' entry '%s': %s"),
               group.c_str(), key.c_str(), value.c_str());
  throw error;
}
