...
GOArchiveEntryFile::GOArchiveEntryFile(GOArchive* archive, const wxString& name, size_t offset, size_t len) :
	m_archiv(archive),
	m_Name(name),
	m_Offset(offset),
	m_Length(len),
...
size_t GOArchiveEntryFile::GetSize()
{
	return m_Length;
}

const wxString GOArchiveEntryFile::GetName()
{
	return m_Name;
}

const wxString GOArchiveEntryFile::GetPath()
{
