...
GOrgueArchiveWriter::GOrgueArchiveWriter() :
	m_Offset(0),
	m_directory(0),
	m_Entries(0),
	m_Names()
...
bool GOrgueArchiveWriter::Open(wxString filename)
{
	m_Offset = 0;
...
bool GOrgueArchiveWriter::Write(const void* data, size_t size)
{
	m_Offset += size;
	return m_File.Write(data, size) == size;
}

bool GOrgueArchiveWriter::Add(wxString name, const GOrgueBuffer<uint8_t>& content)
{
...
bool GOrgueArchiveWriter::Close()
{
	if (!m_File.IsOpened())
		return false;
