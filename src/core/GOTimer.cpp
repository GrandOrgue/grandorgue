...
void GOTimer::SetRelativeTimer(GOTime time, GOTimerCallback* callback, unsigned interval)
{
	time += wxGetLocalTimeMillis();
	SetTimer(time, callback, interval);
}

void GOTimer::SetTimer(GOTime time, GOTimerCallback* callback, unsigned interval)
{
	GOTimerEntry e;
	e.time = time;
	e.callback = callback;
