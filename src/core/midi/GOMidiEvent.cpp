...
void GOMidiEvent::FromMidi(const std::vector<unsigned char>& msg,
                           GOMidiMap& map) {
  ...
  switch (msg[0] & 0xF0) {
    ...
    case 0xF0:
      ...
      if (msg.size() == 9 && msg[0] == 0xF0 && msg[1] == 0x00 &&
          msg[2] == 0x4A && msg[3] == 0x4F && msg[4] == 0x48 &&
          msg[5] == 0x41 && msg[6] == 0x53 && msg[8] == 0xF7) {
        SetKey(msg[7]);
        SetMidiType(MIDI_SYSEX_JOHANNUS_9);
        break;
      }
      if (msg.size() == 11 && msg[0] == 0xF0 && msg[1] == 0x00 &&
          msg[2] == 0x4A && msg[3] == 0x4F && msg[4] == 0x48 &&
          msg[5] == 0x41 && msg[6] == 0x53 && msg[8] == 0x10 &&
          msg[10] == 0xF7) {
        SetKey(msg[9]);
        SetValue(msg[7]);
      ...
      if (msg.size() == 6 && msg[0] == 0xF0 && msg[1] == 0x31 &&
          msg[5] == 0xF7) {
        SetValue(((msg[2] & 0x7F) << 14) | ((msg[3] & 0x7F) << 7) |
                 (msg[4] & 0x7F));
        SetMidiType(MIDI_SYSEX_VISCOUNT);
        break;
      }
      if (msg.size() >= 6 && msg[0] == 0xF0 && msg[1] == 0x7D &&
          msg[2] == 0x47 && msg[3] == 0x4F && msg[msg.size() - 1] == 0xF7) {
        ...
        if (msg[4] == 0x01 && msg.size() == 14) {
          ...
          SetKey(((msg[5] & 0x7F) << 24) | ((msg[6] & 0x7F) << 16) |
                 ((msg[7] & 0x7F) << 8) | (msg[8] & 0x7F));
          SetValue(((msg[9] & 0x7F) << 24) | ((msg[10] & 0x7F) << 16) |
        ...
        if ((msg[4] & 0xF0) == 0x10) {
          ...
          char* buf = b.data();
          for (unsigned i = 0; i < msg.size() - 8; i++) buf[i] = msg[7 + i];
          buf[msg.size() - 8] = 0;
      ...
      if (msg.size() >= 8 && msg[0] == 0xf0 && msg[1] == 0x41 &&
          msg[3] == 0x30 && msg[4] == 0x12 && msg[5] == 0x01 &&
          msg[msg.size() - 2] == GORodgersChecksum(msg, 5, msg.size() - 7) &&
          msg[msg.size() - 1] == 0xf7) {
        SetChannel(msg[2]); /* device*/
        SetKey(msg[6]);
        m_data.resize(msg.size() - 9);
        for (unsigned i = 0; i < m_data.size(); i++) m_data[i] = msg[7 + i];
        SetMidiType(MIDI_SYSEX_RODGERS_STOP_CHANGE);
        break;
      }
      if (msg.size() == 5 && msg[0] == 0xF0 && msg[1] == 0x35 &&
          msg[4] == 0xF7) {
...
void GOMidiEvent::ToMidi(std::vector<std::vector<unsigned char>>& msg,
                         GOMidiMap& map) const {
  msg.resize(0);
  ...
  switch (GetMidiType()) {
    ...
    case MIDI_SYSEX_GO_SETUP: {
      ...
      m[5] = (GetValue() >> 7) & 0x7F;
      m[6] = (GetValue()) & 0x7F;
      for (unsigned i = 0; i < len; i++) m[7 + i] = b[i] & 0x7F;
      m[7 + len] = 0xF7;
      msg.push_back(m);
    }
      return;
    ...
    case MIDI_SYSEX_HW_STRING: {
      ...
      m[2] = 0x19;
      m[3] = (GetKey()) & 0x7F;
      for (unsigned i = 0; i < len; i++) m[4 + i] = b[i] & 0x7F;
      for (unsigned i = len; i < 16; i++) m[4 + i] = ' ';
      m[20] = 0xF7;
      msg.push_back(m);
    }
      return;
    ...
    case MIDI_SYSEX_HW_LCD: {
      ...
      m[4] = (GetKey()) & 0x7F;
      m[5] = (GetChannel()) & 0x7F;
      for (unsigned i = 0; i < len; i++) m[6 + i] = b[i] & 0x7F;
      for (unsigned i = len; i < 32; i++) m[6 + i] = ' ';
      m[38] = 0xF7;
      msg.push_back(m);
    }
      return;
    ...
    case MIDI_SYSEX_RODGERS_STOP_CHANGE: {
      ...
      m[5] = 0x01;
      m[6] = GetKey() & 0x7f;
      for (unsigned i = 0; i < m_data.size(); i++) m[7 + i] = m_data[i];
...
wxString GOMidiEvent::ToString(GOMidiMap& map) const {
  switch (GetMidiType()) {
    ...
    case MIDI_NOTE:
      return wxString::Format(_("note channel: %d key: %d value: %d"),
                              GetChannel(), GetKey(), GetValue());

    case MIDI_AFTERTOUCH:
      return wxString::Format(_("aftertouch channel: %d key: %d value: %d"),
                              GetChannel(), GetKey(), GetValue());

    case MIDI_CTRL_CHANGE:
      return wxString::Format(_("control change channel: %d key: %d value: %d"),
                              GetChannel(), GetKey(), GetValue());

    case MIDI_PGM_CHANGE:
      return wxString::Format(_("program change channel: %d key: %d "),
                              GetChannel(), GetKey());

    case MIDI_RPN:
      return wxString::Format(_("RPN channel: %d key: %d value: %d"),
                              GetChannel(), GetKey(), GetValue());

    case MIDI_NRPN:
      return wxString::Format(_("NRPN change channel: %d key: %d value: %d"),
                              GetChannel(), GetKey(), GetValue());
    ...
    case MIDI_SYSEX_JOHANNUS_11:
      return wxString::Format(_("sysex Johannus 11 bytes value: %d/%d"),
                              GetValue(), GetKey());
    ...
    case MIDI_SYSEX_GO_SAMPLESET:
      return wxString::Format(_("sysex GrandOrgue sampleset %08x%08x"),
                              GetKey(), GetValue());

    case MIDI_SYSEX_GO_SETUP:
      return wxString::Format(
          _("sysex GrandOrgue setup channel: %d NRPN: %d: name: %s"),
          GetChannel(), GetValue(), map.GetElementByID(GetKey()).c_str());

    case MIDI_SYSEX_HW_STRING:
      return wxString::Format(_("sysex Hauptwerk LCD string variable: %d - %s"),
                              GetKey(), GetString().c_str());

    case MIDI_SYSEX_HW_LCD:
      return wxString::Format(
          _("sysex Hauptwerk LCD display: %d color: %d - %s"), GetKey(),
          GetChannel(), GetString().c_str());
