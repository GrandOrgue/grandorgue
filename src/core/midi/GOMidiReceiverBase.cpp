...
void GOMidiReceiverBase::SetElementID(int id)
{
	m_ElementID = id;
}
...
void GOMidiReceiverBase::Load(GOConfigReader& cfg, wxString group, GOMidiMap& map)
{
	m_events.resize(0);
...
int GOMidiReceiverBase::GetTranspose()
{
	return 0;
}

void GOMidiReceiverBase::Save(GOConfigWriter& cfg, wxString group, GOMidiMap& map)
{
...
MIDI_MATCH_TYPE GOMidiReceiverBase::debounce(const GOMidiEvent& e, MIDI_MATCH_TYPE event, unsigned index)
{
...
MIDI_MATCH_TYPE GOMidiReceiverBase::Match(const GOMidiEvent& e, const unsigned midi_map[128], int& key, int& value)
{
	value = 0;
...
void GOMidiReceiverBase::PreparePlayback()
{
	m_Internal.resize(0);
}
