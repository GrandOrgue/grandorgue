...
GOFrameGeneral::GOFrameGeneral(GOCombinationDefinition& general_template, GODefinitionFile* organfile, bool is_setter):
...
void GOFrameGeneral::Load(GOConfigReader& cfg, wxString group)
{
	...
	if (!m_IsSetter)
	{
		...
		for (unsigned i = 0; i < NumberOfStops; i++)
		{
			...
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_STOP, m, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfCouplers; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_COUPLER, m, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfTremulants; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_TREMULANT, -1, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfSwitches; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_SWITCH, -1, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfDivisionalCouplers; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_DIVISIONALCOUPLER, -1, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
