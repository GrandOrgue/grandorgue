...
GOSettingsMidiMatchDialog::GOSettingsMidiMatchDialog(
    ...
    : wxDialog(parent, wxID_ANY, _("MIDI device matching properties"),
  ...
  mainSizer->Add(new wxStaticText(this, wxID_ANY, _("Physical device name:")),
                 0, wxALIGN_RIGHT | wxALL | wxALIGN_CENTER_VERTICAL, 5);
  t_PhysicalName = new wxTextCtrl(this, wxID_ANY, wxEmptyString,
                                  wxDefaultPosition, wxSize(400, -1));
  t_PhysicalName->SetEditable(false);
  mainSizer->Add(t_PhysicalName, 1, wxEXPAND | wxALL, 5);
  mainSizer->Add(new wxStaticText(this, wxID_ANY, _("Logical device name:")), 0,
  ...
  mainSizer->Add(
      new wxStaticText(this, wxID_ANY, _("Physical name regex pattern:")), 0,
      wxALIGN_RIGHT | wxALL | wxALIGN_CENTER_VERTICAL, 5);
  t_regex = new wxTextCtrl(this, ID_REGEX);
  mainSizer->Add(t_regex, 1, wxEXPAND | wxALL, 5);
  topSizer->Add(mainSizer, 0, wxALL | wxEXPAND, 5);

  topSizer->Add(CreateSeparatedButtonSizer(wxOK | wxCANCEL | wxHELP), 0,
...
bool GOSettingsMidiMatchDialog::ValidateLogicalName(wxString& errMsg) {
  ...
  if (isValid && p_OtherDevices)
    ...
    for (const GOMidiDeviceConfig* pDev : *p_OtherDevices)
      if (pDev->m_PhysicalName != m_PhysicalName &&
  ...
  if (isValid && newLogicalName != m_PhysicalName &&
      t_regex->GetValue().IsEmpty()) {
    errMsg =
        _("Regex must be specified when physical and logical device names "
          "differ");
    isValid = false;
  }
...
bool GOSettingsMidiMatchDialog::Validate() {
  ...
  if (!isValid)
    wxMessageBox(errMsg, _("MIDI matching error"),
