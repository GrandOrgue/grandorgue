...
SettingsMidiDevices::SettingsMidiDevices(GOConfig& settings, GOMidi& midi,
                                         wxWindow* parent)
    : wxPanel(parent, wxID_ANY),
      ...
      m_OutDevices(m_Midi.GetOutDevices(), m_config.m_MidiOut, this,
                   ID_OUTDEVICES) {
  ...
  midiPropSizer->Add(m_AutoAddInput = new wxCheckBox(this, ID_AUTO_ADD_MIDI,
                                                     _("Auto add new devices")),
                     0, wxEXPAND | wxALL, 5);
  midiPropSizer->Add(m_CheckOnStartup = new wxCheckBox(
                         this, ID_CHECK_ON_STARTUP, _("Check on startup")),
                     0, wxEXPAND | wxALL, 5);
  ...
  item3->Add(m_InDevices.GetListbox(), 1, wxEXPAND | wxUP | wxRIGHT | wxLEFT,
             5);
  ...
  bottomGb->Add(new wxStaticText(this, wxID_ANY,
                                 _("Send MIDI Recorder Output Stream to:")),
                wxGBPosition(0, 0), wxDefaultSpan,
                wxALIGN_LEFT | wxALIGN_BOTTOM | wxALL);
  m_RecorderDevice = new wxChoice(this, ID_RECORDERDEVICE, wxDefaultPosition,
                                  wxSize(100, wxDefaultCoord));
  bottomGb->Add(m_RecorderDevice, wxGBPosition(1, 0), wxGBSpan(1, 2),
                wxEXPAND | wxALL);
  bottomGb->Add(m_OutDevices.GetMatchingButton(), wxGBPosition(0, 1),
                wxDefaultSpan, wxALIGN_RIGHT | wxDOWN, 5);
...
void SettingsMidiDevices::OnPortChanged(const wxString& portName,
                                        const wxString apiName, bool oldEnabled,
                                        bool newEnabled) {
...
void SettingsMidiDevices::OnInOutDeviceClick(wxCommandEvent& event) {
  ...
  int result = wxGetSingleChoiceIndex(
      ...
      _("MIDI output device"), choices, selection, this);
...
void SettingsMidiDevices::OnInChannelShiftClick(wxCommandEvent& event) {
  GOMidiDeviceConfig& devConf = m_InDevices.GetSelectedDeviceConf();
  int result =
      ::wxGetNumberFromUser(_("A channel offset allows the use of two MIDI\n"
                            ...
                            _("Channel offset:"), devConf.m_PhysicalName,
                            devConf.m_ChannelShift, 0, 15, this);
