...
class GOSettingsPorts {
 private:
  ...
  struct PortItemData : public wxClientData {
    ...
    PortItemData(const wxString& portName,
                 const wxString& apiName = wxEmptyString) {
      m_PortName = portName;
      m_ApiName = apiName;
    }

    bool isItemForPortApi(const wxString& portName,
                          const wxString& apiName = wxEmptyString) {
  ...
  wxString GetPortItemName(const wxString& portName,
                           const wxString& apiName = wxEmptyString) const;

  const wxTreeListItem AddPortItem(const wxTreeListItem& parentItem,
                                   const wxString& portName,
                                   const wxString& apiName = wxEmptyString);

  bool GetPortItemChecked(const wxString& portName,
                          const wxString& apiName = wxEmptyString) const;
 ...
 protected:
  virtual void OnPortChanged(const wxString& portName, const wxString apiName,
                             bool oldEnabled, bool newEnabled) {}

 public:
  GOSettingsPorts(wxWindow* parent, const GOPortFactory& portFactory,
                  const wxString& name);
  ~GOSettingsPorts();
