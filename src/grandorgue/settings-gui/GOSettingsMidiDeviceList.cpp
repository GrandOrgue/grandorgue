...
GOSettingsMidiDeviceList::GOSettingsMidiDeviceList(
    ...
    GOMidiDeviceConfigList& configListPersist, wxWindow* parent, wxWindowID id)
    : m_parent(parent), m_Ports(ports), m_ConfList(configListPersist) {
  m_LbDevices =
      new wxCheckListBox(parent, id, wxDefaultPosition, wxSize(100, 100));
  m_LbDevices->Bind(wxEVT_CHECKLISTBOX, &GOSettingsMidiDeviceList::OnChecked,
                    this);
  m_BMatching = new wxButton(parent, wxID_ANY, _("Matching..."));
  m_BMatching->Bind(wxEVT_BUTTON, &GOSettingsMidiDeviceList::OnMatchingClick,
                    this);
  Init();
}
...
void GOSettingsMidiDeviceList::RefreshDevices(
    const GOPortsConfig& portsConfig, const bool isToAutoEnable,
    ...
    if (port->IsToUse() &&
      ...
      if (!pConfTmp)
        pConfTmp = pConf ? m_ConfListTmp.Append(
                               *pConf,
                               pOutDevList ? &pOutDevList->m_ConfListTmp : NULL)
                         : m_ConfListTmp.Append(port->GetDefaultLogicalName(),
                                                port->GetDefaultRegEx(),
                                                isToAutoEnable, physicalName);
...
void GOSettingsMidiDeviceList::OnMatchingClick(wxCommandEvent& event) {
  ...
  if (dlg.ShowModal() == wxID_OK) {
    dlg.SaveTo(devConf);
    m_ConfListTmp.RemoveByLogicalNameOutOf(devConf.m_LogicalName,
                                           m_ListedConfs);
  }
}
...
void GOSettingsMidiDeviceList::Save(
  ...
  for (GOMidiDeviceConfig* pDevConfTmp : m_ListedConfs) {
    ...
    m_ConfList.RemoveByLogicalNameOutOf(pDevConfTmp->m_LogicalName,
                                        m_ListedConfs);
  }
}
