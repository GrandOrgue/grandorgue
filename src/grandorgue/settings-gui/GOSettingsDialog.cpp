...
GOSettingsDialog::GOSettingsDialog(wxWindow* win, GOSound& sound,
                                   SettingsReasons* reasons)
    : wxPropertySheetDialog(
          win, wxID_ANY, _("Program Settings"), wxDefaultPosition,
          wxDefaultSize,
  ...
  m_MidiDevicePage = new SettingsMidiDevices(m_Sound.GetSettings(),
                                             m_Sound.GetMidi(), notebook);
  ...
  m_MidiMessagePage = new GOSettingsMidiMessage(m_Sound.GetSettings(),
                                                m_Sound.GetMidi(), notebook);
  ...
  notebook->SetSelection(hasReasons ? reasons->operator[](0).m_SettingsPageNum
                                    : 0);
  ...
  if (pButtonSizer) {
    ...
    pButtonSizer->Insert(3, pReasonBtn,
                         wxSizerFlags().Border(wxLEFT | wxRIGHT, 20));
    GetInnerSizer()->Add(pButtonSizer,
