...
void GOBitmapCache::RegisterBitmap(wxImage* bitmap, wxString filename,
                                   wxString maskname) {
...
bool GOBitmapCache::loadFile(wxImage& img, wxString filename) {
  ...
  GOBuffer<char> data;
  if (!file->ReadContent(data)) return false;
...
GOBitmap GOBitmapCache::GetBitmap(wxString filename, wxString maskName) {
  ...
  if (!loadFile(image, filename))
    throw wxString::Format(_("Failed to open the graphic '%s'"),
                           filename.c_str());
  ...
  if (maskName != wxEmptyString) {
    ...
    if (image.GetWidth() != maskimage.GetWidth() ||
      ...
      throw wxString::Format(_("bitmap size of '%s' does not match mask '%s'"),
                             filename.c_str(), maskName.c_str());

    image.SetMaskFromImage(maskimage, 0xFF, 0xFF, 0xFF);
  }

  wxImage* bitmap = new wxImage(image);
  if (bitmap->HasMask()) bitmap->InitAlpha();
