...
void GOGUIEnclosure::Init(GOConfigReader& cfg, wxString group) {
  ...
  for (unsigned i = 1; i <= bitmap_count; i++) {
    wxString bitmap = wxString::Format(wxT(GOBitmapPrefix "enclosure%c%02d"),
                                       wxT('B'), i - 1);
...
void GOGUIEnclosure::Load(GOConfigReader& cfg, wxString group) {
  ...
  unsigned bitmap_count = cfg.ReadInteger(
      ODFSetting, group, wxT("BitmapCount"), 1, 128, false,
      16);  // Allowed number of bitmaps is 1 to 128 (not 127... Midi pedals can
            // give a value from 0 to 127, making 128 levels)
  wxChar style = wxT('A') +
                 cfg.ReadInteger(ODFSetting, group, wxT("EnclosureStyle"), 1, 4,
                                 false, 1) -
                 1;

  for (unsigned i = 1; i <= bitmap_count; i++) {
    wxString bitmap = cfg.ReadStringTrim(
        ODFSetting, group, wxString::Format(wxT("Bitmap%03d"), i), false,
        wxString::Format(wxT(GOBitmapPrefix "enclosure%c%02d"), style, i - 1));
    wxString mask = cfg.ReadStringTrim(ODFSetting, group,
  ...
  m_TextRect = wxRect(x, y, w, h);
  m_TextWidth =
      cfg.ReadInteger(ODFSetting, group, wxT("TextBreakWidth"), 0,
                      m_TextRect.GetWidth(), false, m_TextRect.GetWidth());

  x = cfg.ReadInteger(ODFSetting, group, wxT("MouseRectLeft"), 0,
                      m_BoundingRect.GetWidth() - 1, false, 0);
  y = cfg.ReadInteger(ODFSetting, group, wxT("MouseRectTop"), 0,
                      m_BoundingRect.GetHeight() - 1, false, 13);
  ...
  m_MouseAxisStart = cfg.ReadInteger(ODFSetting, group, wxT("MouseAxisStart"),
                                     0, m_MouseRect.GetHeight(), false,
                                     m_MouseRect.GetHeight() / 3);
  m_MouseAxisEnd = cfg.ReadInteger(ODFSetting, group, wxT("MouseAxisEnd"),
                                   m_MouseAxisStart, m_MouseRect.GetHeight(),
                                   false, m_MouseRect.GetHeight() / 3 * 2);
...
void GOGUIEnclosure::PrepareDraw(double scale, GOBitmap* background) {
  for (unsigned i = 0; i < m_Bitmaps.size(); i++)
    m_Bitmaps[i].PrepareTileBitmap(scale, m_BoundingRect, m_TileOffsetX,
...
bool GOGUIEnclosure::HandleMousePress(int x, int y, bool right,
  ...
  if (right) {
    ...
    if (y <= m_MouseAxisStart && m_MouseAxisStart != 0)
      value = (127 * y / m_MouseAxisStart);
             ...
             m_MouseRect.GetHeight() - m_MouseAxisEnd != 0)
