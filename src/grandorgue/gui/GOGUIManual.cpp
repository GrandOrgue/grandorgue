...
GOGUIManual::GOGUIManual(GOGUIPanel* panel, GOManual* manual,
                         unsigned manual_number)
...
void GOGUIManual::Init(GOConfigReader& cfg, wxString group) {
  const wxChar* keyNames[12] = {wxT("C"),   wxT("Cis"), wxT("D"),   wxT("Dis"),
                                wxT("E"),   wxT("F"),   wxT("Fis"), wxT("G"),
  ...
  for (unsigned i = 0; i < m_Keys.size(); i++) {
    ...
    m_Keys[i].Rect =
        wxRect(x + key_offset, y + key_yoffset, m_Keys[i].OnBitmap.GetWidth(),
               m_Keys[i].OnBitmap.GetHeight());
    if (x + key_offset < 0)
      wxLogWarning(_("Manual key %d outside of the bounding box"),
                   m_Keys[i].MidiNumber);
    ...
    m_Keys[i].MouseRect =
        wxRect(m_Keys[i].Rect.GetX() + mouse_x, m_Keys[i].Rect.GetY() + mouse_y,
               mouse_w, mouse_h);
...
void GOGUIManual::Load(GOConfigReader& cfg, wxString group) {
  ...
  bool color_wooden = cfg.ReadBoolean(ODFSetting, group,
                                      wxT("DispKeyColourWooden"), false, false);
  wxString type = m_ManualNumber ? wxT("Manual") : wxT("Pedal");
  if (color_inverted) type += wxT("Inverted");
  if (color_wooden && m_ManualNumber) type += wxT("Wood");
  unsigned first_midi_note =
      cfg.ReadInteger(ODFSetting, group, wxT("DisplayFirstNote"), 0, 127, false,
  ...
  m_Keys.resize(cfg.ReadInteger(ODFSetting, group, wxT("DisplayKeys"), 1,
                                m_manual->GetNumberOfAccessibleKeys(), false,
                                m_manual->GetNumberOfAccessibleKeys()));
  for (unsigned i = 0; i < m_Keys.size(); i++) {
    unsigned key_nb = i + first_midi_note;
    m_Keys[i].MidiNumber = cfg.ReadInteger(
        ODFSetting, group, wxString::Format(wxT("DisplayKey%03d"), i + 1), 0,
        127, false, key_nb);
    key_nb = cfg.ReadInteger(ODFSetting, group,
  ...
  for (unsigned i = 0; i < m_Keys.size(); i++) {
    ...
    on_file = cfg.ReadStringTrim(ODFSetting, group, wxT("ImageOn_") + base,
                                 false, on_file);
    off_file = cfg.ReadStringTrim(ODFSetting, group, wxT("ImageOff_") + base,
                                  false, off_file);
    on_mask_file = cfg.ReadStringTrim(ODFSetting, group, wxT("MaskOn_") + base,
                                      false, wxEmptyString);
    off_mask_file = cfg.ReadStringTrim(
        ODFSetting, group, wxT("MaskOff_") + base, false, on_mask_file);

    on_file = cfg.ReadStringTrim(ODFSetting, group,
                                 wxString::Format(wxT("Key%03dImageOn"), i + 1),
                                 false, on_file);
    off_file = cfg.ReadStringTrim(
        ODFSetting, group, wxString::Format(wxT("Key%03dImageOff"), i + 1),
        false, off_file);
    on_mask_file = cfg.ReadStringTrim(
        ODFSetting, group, wxString::Format(wxT("Key%03dMaskOn"), i + 1), false,
        on_mask_file);
    off_mask_file = cfg.ReadStringTrim(
        ODFSetting, group, wxString::Format(wxT("Key%03dMaskOff"), i + 1),
        false, off_mask_file);
    ...
    key_width = cfg.ReadInteger(ODFSetting, group, wxT("Width_") + base, 0, 500,
                                false, key_width);
    key_offset = cfg.ReadInteger(ODFSetting, group, wxT("Offset_") + base, -500,
                                 500, false, key_offset);
    key_yoffset = cfg.ReadInteger(ODFSetting, group, wxT("YOffset_") + base, 0,
                                  500, false, key_yoffset);

    key_width = cfg.ReadInteger(ODFSetting, group,
                                wxString::Format(wxT("Key%03dWidth"), i + 1), 0,
                                500, false, key_width);
    key_offset = cfg.ReadInteger(ODFSetting, group,
                                 wxString::Format(wxT("Key%03dOffset"), i + 1),
                                 -500, 500, false, key_offset);
    key_yoffset = cfg.ReadInteger(
        ODFSetting, group, wxString::Format(wxT("Key%03dYOffset"), i + 1), 0,
        500, false, key_yoffset);
    ...
    unsigned mouse_x = cfg.ReadInteger(
        ODFSetting, group, wxString::Format(wxT("Key%03dMouseRectLeft"), i + 1),
        0, m_Keys[i].Rect.GetWidth() - 1, false, 0);
    unsigned mouse_y = cfg.ReadInteger(
        ODFSetting, group, wxString::Format(wxT("Key%03dMouseRectTop"), i + 1),
        0, m_Keys[i].Rect.GetHeight() - 1, false, 0);
    unsigned mouse_w =
        cfg.ReadInteger(ODFSetting, group,
                        wxString::Format(wxT("Key%03dMouseRectWidth"), i + 1),
                        1, m_Keys[i].Rect.GetWidth() - mouse_x, false,
                        m_Keys[i].Rect.GetWidth() - mouse_x);
    unsigned mouse_h =
        cfg.ReadInteger(ODFSetting, group,
                        wxString::Format(wxT("Key%03dMouseRectHeight"), i + 1),
                        1, m_Keys[i].Rect.GetHeight() - mouse_y, false,
...
bool GOGUIManual::HandleMousePress(int x, int y, bool right,
  ...
  if (right) {
    ...
    for (unsigned i = 0; i < m_Keys.size(); i++) {
      if (m_Keys[i].MouseRect.Contains(x, y)) {
        if (i + 1 < m_Keys.size() && m_Keys[i + 1].IsSharp &&
        ...
        m_manual->Set(m_Keys[i].MidiNumber,
