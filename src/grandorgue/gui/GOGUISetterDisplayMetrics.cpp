...
GOGUISetterDisplayMetrics::GOGUISetterDisplayMetrics(
  ...
  switch (type) {
    ...
    case GOGUI_SETTER_FLOATING:
      x_size =
          40 +
          std::max(10 * organfile->GetManual(organfile->GetODFManualCount())
                            ->GetLogicalKeyCount(),
                   (unsigned)10 * 40);
  ...
  int background_img =
      ini.ReadInteger(ODFSetting, wxT("Organ"),
                      wxT("DispConsoleBackgroundImageNum"), 1, 64, false, 32);
  int horiz_img =
      ini.ReadInteger(ODFSetting, wxT("Organ"),
                      wxT("DispKeyHorizBackgroundImageNum"), 1, 64, false, 22);
  int vert_img =
      ini.ReadInteger(ODFSetting, wxT("Organ"),
                      wxT("DispKeyVertBackgroundImageNum"), 1, 64, false, 19);
