...
GOGUIPanel::GOGUIPanel(GODefinitionFile* organfile)
  ...
  for (unsigned i = 0; i < 64; i++)
    m_WoodImages.push_back(
        LoadBitmap(wxString::Format(wxT(GOBitmapPrefix "wood%02d"), i + 1),
                   wxEmptyString));
}
...
void GOGUIPanel::Init(GOConfigReader& cfg, GOGUIDisplayMetrics* metrics,
                      wxString name, wxString group, wxString group_name) {
...
void GOGUIPanel::Load(GOConfigReader& cfg, wxString group) {
  ...
  if (cfg.ReadInteger(ODFSetting, wxT("Panel000"), wxT("NumberOfGUIElements"),
                      0, 999, false, -1) >= 0)
    new_format = true;
  ...
  LoadBackgroundControl(new GOGUIHW1Background(this), cfg,
                        wxString::Format(wxT("---")));
  ...
  for (unsigned i = 0; i < NumberOfImages; i++) {
    LoadControl(new GOGUIImage(this), cfg,
  ...
  if (is_main_panel) {
    for (unsigned i = 0; i < m_organfile->GetEnclosureCount(); i++)
      if (m_organfile->GetEnclosureElement(i)->IsDisplayed(new_format)) {
        LoadControl(
            new GOGUIEnclosure(this, m_organfile->GetEnclosureElement(i)), cfg,
            wxString::Format(wxT("Enclosure%03d"), i + 1));
      }

    if (!new_format) {
      unsigned NumberOfSetterElements =
          cfg.ReadInteger(ODFSetting, panel_group,
                          wxT("NumberOfSetterElements"), 0, 999, false, 0);
    ...
    for (unsigned i = 0; i < m_organfile->GetTremulantCount(); i++)
      if (m_organfile->GetTremulant(i)->IsDisplayed()) {
        LoadControl(new GOGUIButton(this, m_organfile->GetTremulant(i)), cfg,
    ...
    for (unsigned i = 0; i < m_organfile->GetDivisionalCouplerCount(); i++)
      if (m_organfile->GetDivisionalCoupler(i)->IsDisplayed()) {
        LoadControl(new GOGUIButton(this, m_organfile->GetDivisionalCoupler(i)),
    ...
    for (unsigned i = 0; i < m_organfile->GetGeneralCount(); i++)
      if (m_organfile->GetGeneral(i)->IsDisplayed()) {
        LoadControl(new GOGUIButton(this, m_organfile->GetGeneral(i), true),
                    cfg, wxString::Format(wxT("General%03d"), i + 1));
      }

    for (unsigned i = 0; i < m_organfile->GetNumberOfReversiblePistons(); i++)
      if (m_organfile->GetPiston(i)->IsDisplayed()) {
        LoadControl(new GOGUIButton(this, m_organfile->GetPiston(i), true), cfg,
    ...
    for (unsigned i = 0; i < m_organfile->GetSwitchCount(); i++)
      if (m_organfile->GetSwitch(i)->IsDisplayed()) {
        LoadControl(new GOGUIButton(this, m_organfile->GetSwitch(i), false),
                    cfg, wxString::Format(wxT("Switch%03d"), i + 1));
      }

    for (unsigned int i = m_organfile->GetFirstManualIndex();
         i <= m_organfile->GetManualAndPedalCount(); i++) {
      wxString manual_group = wxString::Format(wxT("Manual%03d"), i);
      if (m_organfile->GetManual(i)->IsDisplayed()) {
        LoadBackgroundControl(
            new GOGUIManualBackground(this, m_layout->GetManualNumber()), cfg,
            manual_group);
        LoadControl(new GOGUIManual(this, m_organfile->GetManual(i),
      ...
      for (unsigned j = 0; j < m_organfile->GetManual(i)->GetStopCount(); j++)
        if (m_organfile->GetManual(i)->GetStop(j)->IsDisplayed()) {
          ...
          buffer = wxString::Format(
              ...
              cfg.ReadInteger(ODFSetting, manual_group, buffer, 1, 999));
          LoadControl(
              new GOGUIButton(this, m_organfile->GetManual(i)->GetStop(j)), cfg,
              buffer);
        }
      ...
      for (unsigned j = 0; j < m_organfile->GetManual(i)->GetDivisionalCount();
        ...
        if (m_organfile->GetManual(i)->GetDivisional(j)->IsDisplayed()) {
          ...
          buffer = wxString::Format(
              ...
              cfg.ReadInteger(ODFSetting, manual_group, buffer, 1, 999));
          LoadControl(
              new GOGUIButton(this, m_organfile->GetManual(i)->GetDivisional(j),
                              true),
              cfg, buffer);
        }
    }
    ...
    unsigned NumberOfEnclosures =
        cfg.ReadInteger(ODFSetting, panel_group, wxT("NumberOfEnclosures"), 0,
    ...
    for (unsigned i = 0; i < NumberOfEnclosures; i++) {
      ...
      LoadControl(new GOGUIEnclosure(
                      this, m_organfile->GetEnclosureElement(enclosure_nb - 1)),
                  cfg, panel_prefix + buffer);
    ...
    for (unsigned i = 0; i < NumberOfSetterElements; i++) {
      ...
      if (!control)
        throw(wxString)
            wxString::Format(_("Unkown SetterElement in section %s"),
                             (panel_prefix + buffer).c_str());
      LoadControl(control, cfg, panel_prefix + buffer);
    }

    unsigned NumberOfTremulants =
        cfg.ReadInteger(ODFSetting, panel_group, wxT("NumberOfTremulants"), 0,
    ...
    unsigned NumberOfDivisionalCouplers = cfg.ReadInteger(
        ODFSetting, panel_group, wxT("NumberOfDivisionalCouplers"), 0,
    ...
    for (unsigned i = 0; i < NumberOfDivisionalCouplers; i++) {
      ...
      unsigned coupler_nb =
          cfg.ReadInteger(ODFSetting, panel_group, buffer, 1,
                          m_organfile->GetDivisionalCouplerCount());
      buffer = wxString::Format(wxT("DivisionalCoupler%03d"), coupler_nb);
      LoadControl(new GOGUIButton(
    ...
    unsigned NumberOfGenerals =
        cfg.ReadInteger(ODFSetting, panel_group, wxT("NumberOfGenerals"), 0,
                        m_organfile->GetGeneralCount());
    for (unsigned i = 0; i < NumberOfGenerals; i++) {
      wxString buffer = wxString::Format(wxT("General%03d"), i + 1);
      unsigned general_nb = cfg.ReadInteger(ODFSetting, panel_group, buffer, 1,
                                            m_organfile->GetGeneralCount());
    ...
    unsigned NumberOfReversiblePistons = cfg.ReadInteger(
        ODFSetting, panel_group, wxT("NumberOfReversiblePistons"), 0,
    ...
    unsigned NumberOfSwitches =
        cfg.ReadInteger(ODFSetting, panel_group, wxT("NumberOfSwitches"), 0,
                        m_organfile->GetSwitchCount(), false, 0);
    for (unsigned i = 0; i < NumberOfSwitches; i++) {
      wxString buffer = wxString::Format(wxT("Switch%03d"), i + 1);
      unsigned switch_nb = cfg.ReadInteger(ODFSetting, panel_group, buffer, 1,
    ...
    unsigned nb_manuals =
        cfg.ReadInteger(ODFSetting, panel_group, wxT("NumberOfManuals"), 0,
    ...
    for (unsigned int i = first_manual; i <= nb_manuals; i++) {
      ...
      unsigned manual_nb = cfg.ReadInteger(
          ODFSetting, panel_group, buffer, m_organfile->GetFirstManualIndex(),
          m_organfile->GetManualAndPedalCount());
      buffer = wxString::Format(wxT("Manual%03d"), manual_nb);
      LoadBackgroundControl(new GOGUIManualBackground(this, i), cfg,
                            panel_prefix + buffer);
      LoadControl(new GOGUIManual(this, m_organfile->GetManual(manual_nb),
                                  m_layout->GetManualNumber()),
                  cfg, panel_prefix + buffer);
      m_organfile->MarkSectionInUse(panel_prefix + buffer);
    }
    ...
    for (unsigned j = 0; j < NumberOfCouplers; j++) {
      ...
      unsigned coupler_nb = cfg.ReadInteger(
          ODFSetting, panel_group, buffer, 1,
          m_organfile->GetManual(manual_nb)->GetODFCouplerCount());
      buffer = wxString::Format(wxT("Coupler%03d"), j + 1);
      LoadControl(
          new GOGUIButton(this, m_organfile->GetManual(manual_nb)->GetCoupler(
                                    coupler_nb - 1)),
          cfg, panel_prefix + buffer);
      m_organfile->MarkSectionInUse(
          panel_prefix + wxString::Format(wxT("Manual%03dCoupler%03d"),
                                          manual_nb, coupler_nb));
    }
    ...
    for (unsigned j = 0; j < NumberOfDivisionals; j++) {
      ...
      buffer = wxString::Format(wxT("Divisional%03d"), j + 1);
      LoadControl(
          new GOGUIButton(this,
                          m_organfile->GetManual(manual_nb)->GetDivisional(
                              divisional_nb - 1),
                          true),
          cfg, panel_prefix + buffer);
      m_organfile->MarkSectionInUse(
          panel_prefix + wxString::Format(wxT("Manual%03dDivisional%03d"),
                                          manual_nb, divisional_nb));
    }
  }
  ...
  for (unsigned i = 0; i < NumberOfGUIElements; i++) {
    ...
    if (type == wxT("Divisional")) {
      unsigned manual_nb = cfg.ReadInteger(
          ODFSetting, buffer, wxT("Manual"), m_organfile->GetFirstManualIndex(),
          m_organfile->GetManualAndPedalCount());
      unsigned divisional_nb = cfg.ReadInteger(
          ODFSetting, buffer, wxT("Divisional"), 1,
          m_organfile->GetManual(manual_nb)->GetDivisionalCount());
      LoadControl(
          new GOGUIButton(this,
                          m_organfile->GetManual(manual_nb)->GetDivisional(
      ...
      unsigned enclosure_nb =
          cfg.ReadInteger(ODFSetting, buffer, wxT("Enclosure"), 1,
                          m_organfile->GetEnclosureCount());
      LoadControl(new GOGUIEnclosure(
                      this, m_organfile->GetEnclosureElement(enclosure_nb - 1)),
                  cfg, buffer);
    } else if (type == wxT("Tremulant")) {
      unsigned tremulant_nb =
          cfg.ReadInteger(ODFSetting, buffer, wxT("Tremulant"), 1,
                          m_organfile->GetTremulantCount());
      LoadControl(
          new GOGUIButton(this, m_organfile->GetTremulant(tremulant_nb - 1)),
          cfg, buffer);
      ...
      unsigned coupler_nb =
          cfg.ReadInteger(ODFSetting, buffer, wxT("DivisionalCoupler"), 1,
                          m_organfile->GetDivisionalCouplerCount());
      LoadControl(new GOGUIButton(
                      this, m_organfile->GetDivisionalCoupler(coupler_nb - 1)),
                  cfg, buffer);
    } else if (type == wxT("General")) {
      unsigned general_nb = cfg.ReadInteger(ODFSetting, buffer, wxT("General"),
                                            1, m_organfile->GetGeneralCount());
      LoadControl(
          new GOGUIButton(this, m_organfile->GetGeneral(general_nb - 1), true),
          cfg, buffer);
    } else if (type == wxT("ReversiblePiston")) {
      unsigned piston_nb =
          cfg.ReadInteger(ODFSetting, buffer, wxT("ReversiblePiston"), 1,
                          m_organfile->GetNumberOfReversiblePistons());
      LoadControl(
          new GOGUIButton(this, m_organfile->GetPiston(piston_nb - 1), true),
          cfg, buffer);
    } else if (type == wxT("Switch")) {
      unsigned switch_nb = cfg.ReadInteger(ODFSetting, buffer, wxT("Switch"), 1,
                                           m_organfile->GetSwitchCount());
      LoadControl(
          new GOGUIButton(this, m_organfile->GetSwitch(switch_nb - 1), false),
          cfg, buffer);
      ...
      unsigned manual_nb = cfg.ReadInteger(
          ...
          m_organfile->GetManualAndPedalCount());
      unsigned coupler_nb = cfg.ReadInteger(
          ODFSetting, buffer, wxT("Coupler"), 1,
          m_organfile->GetManual(manual_nb)->GetODFCouplerCount());
      LoadControl(
          new GOGUIButton(this, m_organfile->GetManual(manual_nb)->GetCoupler(
                                    coupler_nb - 1)),
      ...
      unsigned manual_nb = cfg.ReadInteger(
          ...
          m_organfile->GetManualAndPedalCount());
      unsigned stop_nb =
          cfg.ReadInteger(ODFSetting, buffer, wxT("Stop"), 1,
                          m_organfile->GetManual(manual_nb)->GetStopCount());
      LoadControl(
          new GOGUIButton(
              this, m_organfile->GetManual(manual_nb)->GetStop(stop_nb - 1)),
...
GOGUIControl* GOGUIPanel::CreateGUIElement(GOConfigReader& cfg,
                                           wxString group) {
...
void GOGUIPanel::LoadControl(GOGUIControl* control, GOConfigReader& cfg,
                             wxString group) {
  control->Load(cfg, group);
  AddControl(control);
}

void GOGUIPanel::LoadBackgroundControl(GOGUIControl* control,
...
void GOGUIPanel::ReadSizeInfoFromCfg(GOConfigReader& cfg,
                                     bool isOpenByDefault) {
  int x = cfg.ReadInteger(CMBSetting, m_group, wxT("WindowX"), -windowLimit,
                          windowLimit, false, 0);
  int y = cfg.ReadInteger(CMBSetting, m_group, wxT("WindowY"), -windowLimit,
                          windowLimit, false, 0);
  int w = cfg.ReadInteger(CMBSetting, m_group, wxT("WindowWidth"), 0,
                          windowLimit, false, 0);
  int h = cfg.ReadInteger(CMBSetting, m_group, wxT("WindowHeight"), 0,
                          windowLimit, false, 0);

  m_rect = wxRect(x, y, w, h);
  m_DisplayNum = cfg.ReadInteger(CMBSetting, m_group, wxT("DisplayNumber"), -1,
                                 windowLimit, false, -1);
  m_IsMaximized = cfg.ReadBoolean(CMBSetting, m_group, wxT("WindowMaximized"),
                                  false, false);
...
void GOGUIPanel::Save(GOConfigWriter& cfg) {
  ...
  cfg.WriteInteger(m_group, wxT("WindowWidth"),
                   std::min(windowLimit, size.GetWidth()));
  cfg.WriteInteger(m_group, wxT("WindowHeight"),
                   std::min(windowLimit, size.GetHeight()));
}
...
void GOGUIPanel::SendMousePress(int x, int y, bool right,
