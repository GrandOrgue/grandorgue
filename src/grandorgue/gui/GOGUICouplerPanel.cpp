...
GOGUIPanel* GOGUICouplerPanel::CreateCouplerPanel(GOConfigReader& cfg, unsigned manual_nr)
{
	...
	for (unsigned int i = m_organfile->GetFirstManualIndex(); i < m_organfile->GetODFManualCount(); i++)
	{
		...
		coupler->Init(cfg, wxString::Format(wxT("SetterManual%03dCoupler%03dT16"), manual_nr, i), _("16"), false, false, -12, i, GOCoupler::COUPLER_NORMAL);
		...
		coupler = new GOCoupler(m_organfile, manual_nr);
		coupler->Init(cfg, wxString::Format(wxT("SetterManual%03dCoupler%03dT8"), manual_nr, i), manual_nr != i ? _("8") : _("U.O."), manual_nr == i, false, 0, i, GOCoupler::COUPLER_NORMAL);
		...
		coupler = new GOCoupler(m_organfile, manual_nr);
		coupler->Init(cfg, wxString::Format(wxT("SetterManual%03dCoupler%03dT4"), manual_nr, i), _("4"), false, false, 12, i, GOCoupler::COUPLER_NORMAL);
		...
		coupler = new GOCoupler(m_organfile, manual_nr);
		coupler->Init(cfg, wxString::Format(wxT("SetterManual%03dCoupler%03dBAS"), manual_nr, i), _("BAS"), false, false, 0, i, GOCoupler::COUPLER_BASS);
		...
		coupler = new GOCoupler(m_organfile, manual_nr);
		coupler->Init(cfg, wxString::Format(wxT("SetterManual%03dCoupler%03dMEL"), manual_nr, i), _("MEL"), false, false, 0, i, GOCoupler::COUPLER_MELODY);
