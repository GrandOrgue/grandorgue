...
GOGUIManualBackground::GOGUIManualBackground(GOGUIPanel* panel,
...
void GOGUIManualBackground::Layout() {
  ...
  m_VRect = wxRect(m_layout->GetCenterX(), mri.y, m_layout->GetCenterWidth(),
                   mri.height);
  m_VBackground = m_panel->GetWood(m_metrics->GetKeyVertBackgroundImageNum());
  m_HRect =
      wxRect(m_layout->GetCenterX(), mri.piston_y, m_layout->GetCenterWidth(),
