...
void GOGUILabel::Init(GOConfigReader& cfg, wxString group, unsigned x_pos,
                      unsigned y_pos, wxString name, unsigned DispImageNum) {
...
void GOGUILabel::Load(GOConfigReader& cfg, wxString group) {
  ...
  if (!FreeXPlacement) {
    int DispDrawstopCol =
        cfg.ReadInteger(ODFSetting, group, wxT("DispDrawstopCol"), 1,
                        m_metrics->NumberOfDrawstopColsToDisplay(), true, 1);
    ...
    x = cfg.ReadInteger(ODFSetting, group, wxT("DispXpos"), 0,
                        m_metrics->GetScreenWidth(), false, 0);
  }
  ...
  if (!FreeYPlacement) {
    ...
    y = cfg.ReadInteger(ODFSetting, group, wxT("DispYpos"), 0,
                        m_metrics->GetScreenHeight(), false, 0);
  }
  ...
  wxString image_file = cfg.ReadStringTrim(
      ODFSetting, group, wxT("Image"), false,
      wxString::Format(wxT(GOBitmapPrefix "label%02d"), DispImageNum));
  ...
  x = cfg.ReadInteger(ODFSetting, group, wxT("PositionX"), 0,
                      m_metrics->GetScreenWidth(), false, x);
  y = cfg.ReadInteger(ODFSetting, group, wxT("PositionY"), 0,
                      m_metrics->GetScreenHeight(), false, y);
  w = cfg.ReadInteger(ODFSetting, group, wxT("Width"), 1,
                      m_metrics->GetScreenWidth(), false, m_Bitmap.GetWidth());
...
void GOGUILabel::PrepareDraw(double scale, GOBitmap* background) {
  m_Bitmap.PrepareTileBitmap(scale, m_BoundingRect, m_TileOffsetX,
...
bool GOGUILabel::HandleMousePress(int x, int y, bool right,
