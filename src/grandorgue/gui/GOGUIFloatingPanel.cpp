...
GOGUIPanel* GOGUIFloatingPanel::CreateFloatingPanel(GOConfigReader& cfg) {
  ...
  panel->Init(cfg, metrics, _("Coupler manuals and volume"),
              wxT("SetterFloating"), wxT(""));
  ...
  for (unsigned i = m_organfile->GetODFManualCount();
    ...
    wxString group;
    group.Printf(wxT("SetterFloating%03d"),
                 i - m_organfile->GetODFManualCount() + 1);
    ...
    GOGUIManual* manual = new GOGUIManual(panel, m_organfile->GetManual(i),
    ...
    for (unsigned j = 0; j < 10; j++) {
      GODivisional* divisional = new GODivisional(
          m_organfile, m_organfile->GetManual(i)->GetDivisionalTemplate(),
          true);
      divisional->Init(
          cfg, wxString::Format(wxT("Setter%03dDivisional%03d"), i, j + 100), i,
          100 + j, wxString::Format(wxT("%d"), j + 1));
      m_organfile->GetManual(i)->AddDivisional(divisional);

      GOGUIButton* button = new GOGUIButton(panel, divisional, true);
      button->Init(
          cfg, wxString::Format(wxT("Setter%03dDivisional%03d"), i, j + 100),
          j + 1, i - m_organfile->GetODFManualCount());
  ...
  for (unsigned i = 0; i < m_organfile->GetWindchestGroupCount(); i++) {
    ...
    enc->Init(cfg, wxString::Format(wxT("SetterMaster%03d"), i + 1),
              windchest->GetName(), 127);
