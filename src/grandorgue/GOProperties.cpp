	...
	m_organfile(organfile)
{
	...
	sizer->Add(GOPropertiesText(this, 0,  _("Allocated sample memory")), 0, wxTOP, 5);
	float size, size1;
	...
	if (m_organfile->GetMemoryPool().GetMemoryLimit() > 0)
		sizer->Add(GOPropertiesText(this, 0,  wxString::Format(_("%.3f MB of %.3f MB"), size1, size)), 0, wxTOP, 5);
	else
		sizer->Add(GOPropertiesText(this, 0,  wxString::Format(_("%.3f MB (not limited)"), size1)), 0, wxTOP, 5);

	sizer->Add(GOPropertiesText(this, 0,  _("Mapped memory of the cache")), 0, wxTOP, 5);
	size = m_organfile->GetMemoryPool().GetMappedSize() / (1024.0 * 1024.0);
	sizer->Add(GOPropertiesText(this, 0,  wxString::Format(_("%.3f MB"), size)), 0, wxTOP, 5);
	...
	sizer->Add(GOPropertiesText(this, 300, m_organfile->GetOrganPathInfo()), 0, wxLEFT, 10);
