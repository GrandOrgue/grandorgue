...
class GOManual : private GOEventHandler, private GOSaveableObject, private GOPlaybackStateHandler,
...
public:
	...
	void Init(GOConfigReader& cfg, wxString group, int manualNumber, unsigned first_midi, unsigned keys);
	void Load(GOConfigReader& cfg, wxString group, int manualNumber);
	unsigned RegisterCoupler(GOCoupler* coupler);
	void SetKey(unsigned note, unsigned velocity, GOCoupler* prev, unsigned couplerID);
