...
void GOSoundReverb::Setup(GOConfig& settings) {
  ...
  try {
    for (unsigned i = 0; i < m_engine.size(); i++)
      if (m_engine[i]->configure(1, 1, 1000000, settings.SamplesPerBuffer(),
    ...
    wav.ReadSamples(data, GOWave::SF_IEEE_FLOAT, wav.GetSampleRate(),
    ...
    for (unsigned i = 0; i < m_channels; i++) {
      ...
      for (unsigned j = 0; j < l; j += block) {
        m_engine[i]->impdata_create(0, 0, 1, d + j, delay + j,
                                    delay + j + std::min(l - j, block));
      }
    }
    wav.Close();
    for (unsigned i = 0; i < m_engine.size(); i++)
