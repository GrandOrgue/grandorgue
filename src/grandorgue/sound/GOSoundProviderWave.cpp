...
void GOSoundProviderWave::CreateAttack(
    const char* data, GOWave& wave, int attack_start,
    std::vector<GO_WAVE_LOOP> loop_list, int sample_group,
    unsigned bits_per_sample, unsigned channels, bool compress,
    loop_load_type loop_mode, bool percussive, unsigned min_attack_velocity,
    unsigned loop_crossfade_length, unsigned max_released_time) {
  ...
  for (unsigned i = 0; i < loop_list.size(); i++) {
    ...
    if (loop_crossfade_length && loop_list[i].start_sample +
                                         REMAINING_AFTER_CROSSFADE +
  ...
  section->Setup(
      ...
      (GOWave::SAMPLE_FORMAT)bits_per_sample, channels, wave.GetSampleRate(),
      wave.GetLength(), &loops, compress, loop_crossfade_length);
}

void GOSoundProviderWave::CreateRelease(const char* data, GOWave& wave,
                                        int sample_group,
                                        unsigned max_playback_time,
                                        int cue_point, int release_end,
                                        unsigned bits_per_sample,
                                        unsigned channels, bool compress) {
  ...
  section->Setup(
      ...
      (GOWave::SAMPLE_FORMAT)bits_per_sample, channels, wave.GetSampleRate(),
      release_samples, NULL, compress, 0);
}
...
void GOSoundProviderWave::ProcessFile(
    const GOFilename& filename, std::vector<GO_WAVE_LOOP> loops, bool is_attack,
    bool is_release, int sample_group, unsigned max_playback_time,
    int attack_start, int cue_point, int release_end, unsigned bits_per_sample,
    int load_channels, bool compress, loop_load_type loop_mode, bool percussive,
    unsigned min_attack_velocity, bool use_pitch,
    unsigned loop_crossfade_length, unsigned max_released_time) {
  wxLogDebug(_("Loading file %s"), filename.GetTitle().c_str());
  ...
  unsigned totalDataSize = wave.GetLength() *
                           GetBytesPerSample(bits_per_sample) *
                           wave.GetChannels();
  ...
  wave.ReadSamples(data.get(), (GOWave::SAMPLE_FORMAT)bits_per_sample,
                   wave.GetSampleRate(), wave_channels);

  if (is_attack)
    CreateAttack(data.get(), wave, attack_start, loops, sample_group,
                 bits_per_sample, channels, compress, loop_mode, percussive,
                 min_attack_velocity, loop_crossfade_length, max_released_time);

  if (is_release && (!is_attack || (wave.GetNbLoops() > 0 &&
                                    wave.HasReleaseMarker() && !percussive)))
    CreateRelease(data.get(), wave, sample_group, max_playback_time, cue_point,
                  release_end, bits_per_sample, channels, compress);
}
...
void GOSoundProviderWave::LoadFromFile(
    ...
    std::vector<release_load_info> releases, unsigned bits_per_sample,
    int load_channels, bool compress, loop_load_type loop_mode,
    unsigned attack_load, unsigned release_load, int midi_key_number,
    unsigned loop_crossfade_length, unsigned release_crossfase_length) {
  ClearData();
  ...
  if (!release_load)
    for (int k = -1; k < 2; k++) {
      ...
      for (unsigned i = 0; i < releases.size(); i++)
        if ((unsigned)releases[i].max_playback_time > longest &&
  ...
  if (!attack_load)
    for (int k = -1; k < 2; k++) {
      ...
      for (unsigned i = 0; i < attacks.size(); i++) {
        ...
        if (attacks[i].sample_group != k || best_idx == -1 ||
            best_idx == (int)i)
          continue;
  ...
  try {
    for (unsigned i = 0; i < attacks.size(); i++) {
      ...
      ProcessFile(attacks[i].filename, loops, true, attacks[i].load_release,
                  attacks[i].sample_group, attacks[i].max_playback_time,
                  attacks[i].attack_start, attacks[i].cue_point,
                  attacks[i].release_end, bits_per_sample, load_channels,
                  compress, loop_mode, attacks[i].percussive,
                  attacks[i].min_attack_velocity, load_first_attack,
                  loop_crossfade_length, attacks[i].max_released_time);
      load_first_attack = false;
    }

    for (unsigned i = 0; i < releases.size(); i++) {
      std::vector<GO_WAVE_LOOP> loops;
      ProcessFile(releases[i].filename, loops, false, true,
                  releases[i].sample_group, releases[i].max_playback_time, 0,
                  releases[i].cue_point, releases[i].release_end,
                  bits_per_sample, load_channels, compress, loop_mode, true, 0,
                  false, loop_crossfade_length, 0);
    }
