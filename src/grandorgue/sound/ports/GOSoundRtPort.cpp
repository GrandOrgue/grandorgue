...
void GOSoundRtPort::Open() {
  Close();
  if (!m_rtApi)
    throw wxString::Format(_("Audio device %s not initialised"),
                           m_Name.c_str());
  ...
  try {
    ...
    m_rtApi->openStream(&aOutputParam, NULL, RTAUDIO_FLOAT32, m_SampleRate,
                        &samples_per_buffer, &Callback, this, &aOptions);
    ...
    if (samples_per_buffer != m_SamplesPerBuffer) {
      if (samples_per_buffer != m_SamplesPerBuffer)
        throw wxString::Format(
            ...
            m_Name.c_str(), samples_per_buffer);
    }
    m_IsOpen = true;
  } catch (RtAudioError &e) {
    wxString error = wxString::FromAscii(e.getMessage().c_str());
...
void GOSoundRtPort::StartStream() {
  ...
  if (m_rtApi->getStreamSampleRate() != m_SampleRate)
    throw wxString::Format(_("Sample rate of device %s changed"),
                           m_Name.c_str());
}
...
int GOSoundRtPort::Callback(void *outputBuffer, void *inputBuffer,
...
wxString GOSoundRtPort::getName(RtAudio *rt_api, unsigned index) {
  ...
  return GOSoundPortFactory::getInstance().ComposeDeviceName(PORT_NAME, apiName,
                                                             devName);
}
...
GOSoundPort *GOSoundRtPort::create(const GOPortsConfig &portsConfig,
  ...
  if (portsConfig.IsEnabled(PORT_NAME)) try {
      ...
      for (unsigned k = 0; k < rtaudio_apis.size(); k++) {
        ...
        if (portsConfig.IsEnabled(PORT_NAME, rtApiName) &&
          ...
          try {
            ...
            for (unsigned i = 0; i < deviceCount; i++) {
              ...
              if (devName == name ||
                  devName + GOPortFactory::c_NameDelim == name ||
                  (apiName.IsEmpty() &&
                   get_oldstyle_name(apiIndex, rtApi, i) == name)) {
...
void GOSoundRtPort::addDevices(const GOPortsConfig &portsConfig,
