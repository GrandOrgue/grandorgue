...
GOSoundGroupWorkItem::GOSoundGroupWorkItem(GOSoundEngine& sound_engine,
...
void GOSoundGroupWorkItem::ProcessList(GOSoundSamplerList& list, bool toDropOld,
                                       float* output_buffer) {
  ...
  while ((sampler = list.Get())) {
    ...
    if (windchest &&
        m_engine.ProcessSampler(output_buffer, sampler, m_SamplesPerBuffer,
                                windchest->GetVolume()))
      Add(sampler);
  }
}
...
void GOSoundGroupWorkItem::Run(GOSoundThread* pThread) {
  if (m_Done == 3)  // has already processed in this period
    return;
  {
    GOMutexLocker locker(m_Mutex, false,
                         "GOSoundGroupWorkItem::Run.beforeProcess", pThread);
  ...
  {
    GOMutexLocker locker(m_Mutex, false,
                         "GOSoundGroupWorkItem::Run.afterProcess", pThread);
...
void GOSoundGroupWorkItem::Finish(bool stop, GOSoundThread* pThread) {
  ...
  {
    GOMutexLocker locker(m_Mutex, false, "GOSoundGroupWorkItem::Finish",
                         pThread);
