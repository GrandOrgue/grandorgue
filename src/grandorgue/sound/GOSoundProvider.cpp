...
const GOAudioSection* GOSoundProvider::GetAttack(unsigned velocity,
                                                 unsigned released_time) const {
  ...
  for (unsigned i = 0; i < m_Attack.size(); i++) {
    ...
    if (best_match == -1)
      best_match = idx;
    else if (m_AttackInfo[best_match].min_attack_velocity <
...
const GOAudioSection* GOSoundProvider::GetRelease(
  ...
  for (unsigned i = 0; i < m_Release.size(); i++) {
    ...
    if (m_ReleaseInfo[idx].sample_group !=
        m_AttackInfo[attack_idx].sample_group)
      continue;
    if (m_ReleaseInfo[idx].max_playback_time < time) continue;
    if (best_match == -1)
      best_match = idx;
    else if (m_ReleaseInfo[best_match].max_playback_time >
