...
void GORank::Init(GOConfigReader& cfg, wxString group, wxString name, int first_midi_note_number, unsigned windchest_nr)
{
...
void GORank::Load(GOConfigReader& cfg, wxString group, int first_midi_note_number)
{
	m_organfile->RegisterSaveableObject(this);
	m_group = group;
	...
	for (unsigned i = 0; i < number_of_logical_pipes; i++)
	{
		...
		if (name == wxT("DUMMY"))
		{
			m_Pipes.push_back(new GODummyPipe (m_organfile, this, m_FirstMidiNoteNumber + i));
		}
		else if (name.StartsWith(wxT("REF:")))
		{
			m_Pipes.push_back(new GOReferencePipe (m_organfile, this, m_FirstMidiNoteNumber + i));
		}
		else
		{
			m_Pipes.push_back(new GOSoundingPipe (m_organfile, this, m_Percussive, m_WindchestGroup, m_FirstMidiNoteNumber + i, m_HarmonicNumber, m_PitchCorrection, m_MinVolume, m_MaxVolume, m_RetuneRank));
		}
...
GOPipe* GORank::GetPipe(unsigned index)
{
	return m_Pipes[index];
}

unsigned GORank::GetPipeCount()
{
	return m_Pipes.size();
}

const wxString& GORank::GetName()
{
	return m_Name;
}

GOPipeConfigNode& GORank::GetPipeConfig()
{
	return m_PipeConfig;
}
...
wxString GORank::GetMidiType()
{
	return _("Rank");
}

wxString GORank::GetMidiName()
{
	return m_Name;
}
...
wxString GORank::GetElementStatus()
{
	return _("-");
}
