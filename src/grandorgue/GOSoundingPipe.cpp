...
GOSoundingPipe::GOSoundingPipe(GODefinitionFile* organfile, GORank* rank,
                               bool percussive, int sampler_group_id,
...
void GOSoundingPipe::LoadAttack(GOConfigReader& cfg, wxString group,
                                wxString prefix) {
  attack_load_info ainfo;
  ainfo.filename.Assign(cfg.ReadStringTrim(ODFSetting, group, prefix),
                        m_organfile);
  ...
  ainfo.max_playback_time =
      cfg.ReadInteger(ODFSetting, group, prefix + wxT("MaxKeyPressTime"), -1,
                      100000, false, -1);
  ainfo.cue_point = cfg.ReadInteger(ODFSetting, group, prefix + wxT("CuePoint"),
                                    -1, MAX_SAMPLE_LENGTH, false, -1);
  ...
  ainfo.max_released_time = cfg.ReadInteger(
      ODFSetting, group, prefix + wxT("MaxTimeSinceLastRelease"), -1, 100000,
      false, -1);
  ainfo.attack_start =
      cfg.ReadInteger(ODFSetting, group, prefix + wxT("AttackStart"), 0,
                      MAX_SAMPLE_LENGTH, false, 0);
  ainfo.release_end =
      cfg.ReadInteger(ODFSetting, group, prefix + wxT("ReleaseEnd"), -1,
                      MAX_SAMPLE_LENGTH, false, -1);
  ...
  for (unsigned j = 0; j < loop_cnt; j++) {
    ...
    linfo.loop_start =
        cfg.ReadInteger(ODFSetting, group,
                        prefix + wxString::Format(wxT("Loop%03dStart"), j + 1),
                        0, MAX_SAMPLE_LENGTH, false, 0);
    linfo.loop_end = cfg.ReadInteger(
        ODFSetting, group, prefix + wxString::Format(wxT("Loop%03dEnd"), j + 1),
        linfo.loop_start + 1, MAX_SAMPLE_LENGTH, true);
...
void GOSoundingPipe::Init(GOConfigReader& cfg, wxString group, wxString prefix,
                          wxString filename) {
...
void GOSoundingPipe::Load(GOConfigReader& cfg, wxString group,
                          wxString prefix) {
  m_organfile->RegisterCacheObject(this);
  m_Filename = cfg.ReadStringTrim(ODFSetting, group, prefix);
  m_PipeConfig.Load(cfg, group, prefix);
  m_HarmonicNumber =
      cfg.ReadInteger(ODFSetting, group, prefix + wxT("HarmonicNumber"), 1,
                      1024, false, m_HarmonicNumber);
  m_PitchCorrection =
      cfg.ReadFloat(ODFSetting, group, prefix + wxT("PitchCorrection"), -1800,
                    1800, false, m_PitchCorrection);
  m_SamplerGroupID = cfg.ReadInteger(
      ODFSetting, group, prefix + wxT("WindchestGroup"), 1,
      m_organfile->GetWindchestGroupCount(), false, m_SamplerGroupID);
  m_Percussive = cfg.ReadBoolean(ODFSetting, group, prefix + wxT("Percussive"),
                                 false, m_Percussive);
  ...
  m_ReleaseCrossfadeLength =
      cfg.ReadInteger(ODFSetting, group, prefix + wxT("ReleaseCrossfadeLength"),
                      0, 200, false, 0);
  ...
  for (unsigned i = 0; i < release_count; i++) {
    ...
    rinfo.filename.Assign(cfg.ReadStringTrim(ODFSetting, group, p),
                          m_organfile);
    ...
    rinfo.cue_point = cfg.ReadInteger(ODFSetting, group, p + wxT("CuePoint"),
                                      -1, MAX_SAMPLE_LENGTH, false, -1);
    rinfo.release_end =
        cfg.ReadInteger(ODFSetting, group, p + wxT("ReleaseEnd"), -1,
                        MAX_SAMPLE_LENGTH, false, -1);

    m_ReleaseInfo.push_back(rinfo);
  }

  m_MinVolume = cfg.ReadFloat(ODFSetting, group, wxT("MinVelocityVolume"), 0,
                              1000, false, m_MinVolume);
  m_MaxVolume = cfg.ReadFloat(ODFSetting, group, wxT("MaxVelocityVolume"), 0,
                              1000, false, m_MaxVolume);
  m_SoundProvider.SetVelocityParameter(m_MinVolume, m_MaxVolume);
  m_PipeConfig.SetName(
      wxString::Format(_("%d: %s"), m_MidiKeyNumber, m_Filename.c_str()));
}
...
void GOSoundingPipe::LoadData() {
  try {
    m_SoundProvider.LoadFromFile(
        m_AttackInfo, m_ReleaseInfo, m_PipeConfig.GetEffectiveBitsPerSample(),
        ...
        m_PipeConfig.GetEffectiveReleaseLoad(), m_SampleMidiKeyNumber,
        m_LoopCrossfadeLength, m_ReleaseCrossfadeLength);
    Validate();
  } catch (wxString str) {
...
void GOSoundingPipe::Validate() {
  ...
  if (m_RetunePipe && m_SoundProvider.GetMidiKeyNumber() == 0 &&
      m_SoundProvider.GetMidiPitchFract() == 0 && m_SampleMidiKeyNumber == -1) {
  ...
  if (!m_RetunePipe)
    ...
    offset = m_SoundProvider.GetMidiKeyNumber() +
             ...
             (m_SoundProvider.GetMidiPitchFract() -
              m_PipeConfig.GetDefaultTuning() + m_PitchCorrection) /
                 100.0 -
             m_MidiKeyNumber;
  if (offset < -18 || offset > 18) {
    wxLogError(_("rank %s pipe %s: temperament would retune pipe by more than "
                 "1800 cent"),
               m_Rank->GetName().c_str(), GetLoadTitle().c_str());
    return;
  }
  if (offset < -6 || offset > 6) {
    wxLogWarning(_("rank %s pipe %s: temperament would retune pipe by more "
                   "than 600 cent"),
                 m_Rank->GetName().c_str(), GetLoadTitle().c_str());
    return;
  }
...
void GOSoundingPipe::SetOn(unsigned velocity) {
  m_Sampler = m_organfile->StartSample(
      GetSoundProvider(), m_SamplerGroupID, m_AudioGroupID, velocity,
      m_PipeConfig.GetEffectiveDelay(), m_LastStop);
...
void GOSoundingPipe::UpdateAmplitude() {
  m_SoundProvider.SetAmplitude(m_PipeConfig.GetEffectiveAmplitude(),
                               m_PipeConfig.GetEffectiveGain());
}

void GOSoundingPipe::UpdateTuning() {
  m_SoundProvider.SetTuning(m_PipeConfig.GetEffectiveTuning() +
                            m_TemperamentOffset);
}
...
void GOSoundingPipe::SetTemperament(const GOTemperament& temperament) {
  if (!m_RetunePipe)
    ...
    m_TemperamentOffset = temperament.GetOffset(
        m_organfile->GetIgnorePitch(), m_MidiKeyNumber,
        m_SoundProvider.GetMidiKeyNumber(), m_SoundProvider.GetMidiPitchFract(),
        m_HarmonicNumber, m_PitchCorrection, m_PipeConfig.GetDefaultTuning());
  UpdateTuning();
}
