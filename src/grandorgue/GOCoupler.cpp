...
void GOCoupler::Init(GOConfigReader& cfg, wxString group, wxString name,
                     bool unison_off, bool recursive, int keyshift,
                     int dest_manual, GOCouplerType coupler_type) {
  ...
  if (!m_UnisonOff)
    m_CouplerID =
        m_organfile->GetManual(m_DestinationManual)->RegisterCoupler(this);
}
...
void GOCoupler::Load(GOConfigReader& cfg, wxString group) {
  ...
  m_DestinationManual =
      cfg.ReadInteger(ODFSetting, group, wxT("DestinationManual"),
                      m_organfile->GetFirstManualIndex(),
                      m_organfile->GetManualAndPedalCount(), !m_UnisonOff, 0);
  ...
  if (m_UnisonOff) {
    ...
    if (!m_organfile->GetSettings().ODFCheck()) {
      cfg.ReadBoolean(ODFSetting, group,
                      wxT("CoupleToSubsequentUnisonIntermanualCouplers"), false,
                      false);
      cfg.ReadBoolean(ODFSetting, group,
                      wxT("CoupleToSubsequentUpwardIntermanualCouplers"), false,
                      false);
      cfg.ReadBoolean(ODFSetting, group,
                      wxT("CoupleToSubsequentDownwardIntermanualCouplers"),
                      false, false);
      cfg.ReadBoolean(ODFSetting, group,
                      wxT("CoupleToSubsequentUpwardIntramanualCouplers"), false,
                      false);
      cfg.ReadBoolean(ODFSetting, group,
                      wxT("CoupleToSubsequentDownwardIntramanualCouplers"),
                      false, false);
      cfg.ReadInteger(ODFSetting, group, wxT("FirstMIDINoteNumber"), 0, 127,
                      false, 0);
      cfg.ReadInteger(ODFSetting, group, wxT("NumberOfKeys"), 0, 127, false,
                      127);
    }
  } else {
    m_CouplerType = (GOCouplerType)cfg.ReadEnum(
        ODFSetting, group, wxT("CouplerType"), m_coupler_types,
        sizeof(m_coupler_types) / sizeof(m_coupler_types[0]), false,
        COUPLER_NORMAL);
    if (m_CouplerType == COUPLER_BASS || m_CouplerType == COUPLER_MELODY) {
      ...
      m_CoupleToSubsequentUnisonIntermanualCouplers = cfg.ReadBoolean(
          ODFSetting, group, wxT("CoupleToSubsequentUnisonIntermanualCouplers"),
          true, false);
      m_CoupleToSubsequentUpwardIntermanualCouplers = cfg.ReadBoolean(
          ODFSetting, group, wxT("CoupleToSubsequentUpwardIntermanualCouplers"),
          true, false);
      m_CoupleToSubsequentDownwardIntermanualCouplers = cfg.ReadBoolean(
          ODFSetting, group,
          wxT("CoupleToSubsequentDownwardIntermanualCouplers"), true, false);
      m_CoupleToSubsequentUpwardIntramanualCouplers = cfg.ReadBoolean(
          ODFSetting, group, wxT("CoupleToSubsequentUpwardIntramanualCouplers"),
          true, false);
      m_CoupleToSubsequentDownwardIntramanualCouplers = cfg.ReadBoolean(
          ODFSetting, group,
          wxT("CoupleToSubsequentDownwardIntramanualCouplers"), true, false);
    }
    m_FirstMidiNote = cfg.ReadInteger(
        ODFSetting, group, wxT("FirstMIDINoteNumber"), 0, 127, false, 0);
    m_NumberOfKeys = cfg.ReadInteger(ODFSetting, group, wxT("NumberOfKeys"), 0,
                                     127, false, 127);
  }
...
void GOCoupler::SetKey(unsigned note, const std::vector<unsigned>& velocities,
  ...
  for (unsigned i = 0; i < velocities.size(); i++) {
    ...
    if (prev) {
      if ((prev->m_CoupleToSubsequentUnisonIntermanualCouplers &&
           m_DestinationKeyshift == 0) ||
          (prev->m_CoupleToSubsequentDownwardIntramanualCouplers &&
           m_DestinationKeyshift < 0 && !IsIntermanual()) ||
          (prev->m_CoupleToSubsequentUpwardIntramanualCouplers &&
           m_DestinationKeyshift > 0 && !IsIntermanual()) ||
          (prev->m_CoupleToSubsequentDownwardIntermanualCouplers &&
           m_DestinationKeyshift < 0 && IsIntermanual()) ||
          (prev->m_CoupleToSubsequentUpwardIntermanualCouplers &&
           m_DestinationKeyshift > 0 && IsIntermanual()))
        ;
      else
        continue;
