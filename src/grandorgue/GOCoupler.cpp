...
void GOCoupler::Init(GOConfigReader& cfg, wxString group, wxString name, bool unison_off, bool recursive, int keyshift, int dest_manual, GOCouplerType coupler_type)
{
...
void GOCoupler::Load(GOConfigReader& cfg, wxString group)
{
	...
	if (m_UnisonOff)
	{
		...
		if (!m_organfile->GetSettings().ODFCheck())
		{
			...
			cfg.ReadInteger(ODFSetting, group, wxT("FirstMIDINoteNumber"), 0, 127, false, 0); 
			cfg.ReadInteger(ODFSetting, group, wxT("NumberOfKeys"), 0, 127, false, 127);
		}
	}
	else
	{
		m_CouplerType = (GOCouplerType)cfg.ReadEnum(ODFSetting, group, wxT("CouplerType"), m_coupler_types, sizeof(m_coupler_types) / sizeof(m_coupler_types[0]), false, COUPLER_NORMAL);
...
void GOCoupler::SetKey(unsigned note, const std::vector<unsigned>& velocities, const std::vector<GOCoupler*>& couplers)
{
...
bool GOCoupler::IsUnisonOff()
{
	return m_UnisonOff;
}

wxString GOCoupler::GetMidiType()
{
	return _("Coupler");
}
