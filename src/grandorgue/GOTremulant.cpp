...
bool GOTremulant::SaveCache(GOCacheWriter& cache)
{
	return true;
}
...
const wxString& GOTremulant::GetLoadTitle()
{
	return m_Name;
}

void GOTremulant::Load(GOConfigReader& cfg, wxString group, int sampler_group_id)
{
	m_TremulantType = (GOTremulantType)cfg.ReadEnum(ODFSetting, group, wxT("TremulantType"), m_tremulant_types, sizeof(m_tremulant_types) / sizeof(m_tremulant_types[0]), false, GOSynthTrem);
...
GOTremulantType GOTremulant::GetTremulantType()
{
	return m_TremulantType;
}

wxString GOTremulant::GetMidiType()
{
	return _("Tremulant");
}
