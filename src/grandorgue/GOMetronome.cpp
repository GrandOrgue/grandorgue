...
void GOMetronome::Load(GOConfigReader& cfg) {
  ...
  m_BPM = cfg.ReadInteger(CMBSetting, m_group, wxT("BPM"), 1, 500, false,
                          m_organfile->GetSettings().MetronomeBPM());
  m_MeasureLength =
      cfg.ReadInteger(CMBSetting, m_group, wxT("MeasureLength"), 0, 32, false,
  ...
  GOSoundingPipe* pipe;
  pipe = new GOSoundingPipe(m_organfile, m_rank, true, samplegroup, 36, 8, 0,
                            100, 100, false);
  m_rank->AddPipe(pipe);
  pipe->Init(cfg, wxT("MetronomSounds"), wxT("A"),
             wxT("sounds\\metronome\\beat.wv"));
  pipe = new GOSoundingPipe(m_organfile, m_rank, true, samplegroup, 37, 8, 0,
                            100, 100, false);
  m_rank->AddPipe(pipe);
  pipe->Init(cfg, wxT("MetronomSounds"), wxT("B"),
