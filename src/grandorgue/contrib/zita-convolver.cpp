...
void Convlevel::impdata_create(
  ...
  for (k = 0; k < _npar; k++) {
    ...
    if ((i0 < n) && (i1 > 0)) {
      if (!(M->_fftb[k])) {
        M->_fftb[k] = (fftwf_complex *)(alloc_aligned(
          (_parsize + 1) * sizeof(fftwf_complex)));
      }
      if (data) {
...
void Convlevel::print(FILE *F) {
  fprintf(
    F,
    "prio = %4d, offs = %6d,  parsize = %5d,  npar = %3d\n",
    _prio,
    _offs,
    _parsize,
    _npar);
...
Convlevel::findmacnode(unsigned int inp, unsigned int out, bool create) {
  ...
  if (!X) {
    ...
    for (i = 0; i < _npar; i++) {
      X->_ffta[i] = (fftwf_complex *)(alloc_aligned(
        (_parsize + 1) * sizeof(fftwf_complex)));
    }
  }
