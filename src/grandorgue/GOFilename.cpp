...
void GOFilename::SetPath(const wxString& base, const wxString& file) {
  ...
  if (path_name.GetLongPath() != path_name.GetFullPath()) {
    wxLogWarning(_("Filename '%s' not compatible with case sensitive systems"),
                 file.c_str());
  }
...
void GOFilename::Assign(const wxString& name, GODefinitionFile* organfile) {
  m_Name = name;
  if (organfile->GetSettings().ODFCheck() &&
      name.Find(wxT('/')) != wxNOT_FOUND) {
    wxLogWarning(_("Filename '%s' contains non-portable directory seperator /"),
                 name.c_str());
  }
...
void GOFilename::AssignResource(const wxString& name,
