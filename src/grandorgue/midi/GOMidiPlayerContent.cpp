...
void GOMidiPlayerContent::Reset()
{
	m_Pos = 0;
}

bool GOMidiPlayerContent::IsLoaded()
{
	return m_Events.size() > 0;
}

void GOMidiPlayerContent::ReadFileContent(GOMidiFileReader& reader, std::vector<GOMidiEvent>& events)
{
	unsigned pos = 0;
	GOMidiEvent e;
...
void GOMidiPlayerContent::SetupManual(GOMidiMap& map, unsigned channel, wxString ID)
{
	GOMidiEvent e;
	e.SetTime(0);
...
bool GOMidiPlayerContent::Load(GOMidiFileReader& reader, GOMidiMap& map, unsigned manuals, bool pedal)
{
	Clear();
