...
MIDIEventKeyDialog::MIDIEventKeyDialog(wxWindow* parent, GOKeyReceiver* event)
  ...
  wxBoxSizer* sizer = new wxStaticBoxSizer(wxVERTICAL, this,
                                           m_key.GetType() == KEY_RECV_ENCLOSURE
                                               ? _("&Plus-Shortcut:")
                                               : _("&Shortcut:"));
...
void MIDIEventKeyDialog::Listen(bool enable) {
  if (enable) {
    ...
    m_listen->GetEventHandler()->Connect(
        wxEVT_KEY_DOWN, wxKeyEventHandler(MIDIEventKeyDialog::OnKeyDown), NULL,
        this);
    if (m_minuslisten)
      m_minuslisten->GetEventHandler()->Connect(
          wxEVT_KEY_DOWN, wxKeyEventHandler(MIDIEventKeyDialog::OnKeyDown),
          NULL, this);
  } else {
