...
GOMidiReceiver::GOMidiReceiver(GODefinitionFile* organfile,
...
void GOMidiReceiver::Load(GOConfigReader& cfg, wxString group, GOMidiMap& map) {
  if (m_organfile && !m_organfile->GetSettings().ODFCheck()) {
    ...
    if (m_type == MIDI_RECV_DRAWSTOP)
      cfg.ReadInteger(ODFSetting, group, wxT("StopControlMIDIKeyNumber"), -1,
                      127, false);
    if (m_type == MIDI_RECV_BUTTON)
      cfg.ReadInteger(ODFSetting, group, wxT("MIDIProgramChangeNumber"), 0, 128,
                      false);
  }
