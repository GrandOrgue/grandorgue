...
class GOMidiRtPortFactory {
 ...
 public:
  ...
  void addMissingInDevices(GOMidi* midi, const GOPortsConfig& portsConfig,
                           ptr_vector<GOMidiPort>& ports);
  void addMissingOutDevices(GOMidi* midi, const GOPortsConfig& portsConfig,
                            ptr_vector<GOMidiPort>& ports);

  static wxString getApiName(RtMidi::Api api);
  static const std::vector<wxString>& getApis();

  wxString GetDefaultLogicalName(RtMidi::Api api, const wxString& deviceName,
                                 const wxString& fullName);
  wxString GetDefaultRegEx(RtMidi::Api api, const wxString& deviceName,
                           const wxString& fullName);
};
