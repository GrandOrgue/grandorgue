...
GOMidiRtPortFactory* GOMidiRtPortFactory::getInstance() {
  if (!instance) instance = new GOMidiRtPortFactory();
  return instance;
}
...
void GOMidiRtPortFactory::addMissingOutDevices(GOMidi* midi,
                                               const GOPortsConfig& portsConfig,
                                               ptr_vector<GOMidiPort>& ports) {
  GOMidiPortFactory& portFactory(GOMidiPortFactory::getInstance());
...
wxString GOMidiRtPortFactory::GetDefaultLogicalName(RtMidi::Api api,
                                                    const wxString& deviceName,
...
wxString GOMidiRtPortFactory::GetDefaultRegEx(RtMidi::Api api,
                                              const wxString& deviceName,
                                              const wxString& fullName) {
  wxString regEx;
  ...
  switch (api) {
    ...
    case RtMidi::Api::UNIX_JACK:
      if (m_JackDevnamePattern.Matches(deviceName))
        ...
        regEx = wxString::Format(wxT("Midi-Bridge:%s:(%s_%s) %s"),
