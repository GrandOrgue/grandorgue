...
MIDIEventSendDialog::MIDIEventSendDialog(wxWindow* parent, GOMidiSender* event,
  ...
  m_key = new wxSpinCtrl(this, ID_KEY, wxEmptyString, wxDefaultPosition,
  ...
  sizer->Add(box);
  m_StartValue =
      new wxSpinCtrl(this, ID_START, wxEmptyString, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 0, 127);
  ...
  m_LengthValue =
      new wxSpinCtrl(this, ID_LENGTH, wxEmptyString, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 0, 127);
  ...
  if (m_midi.GetType() == MIDI_SEND_LABEL ||
    ...
    m_eventtype->Append(_("SYSEX Hauptwerk 16 Byte String Value"),
                        MIDI_S_HW_STRING);
  }
  m_eventtype->Append(_("SYSEX Hauptwerk 32 Byte LCD Name"),
                      MIDI_S_HW_NAME_LCD);
  m_eventtype->Append(_("SYSEX Hauptwerk 16 Byte String Name"),
                      MIDI_S_HW_NAME_STRING);
  if (m_midi.GetType() == MIDI_SEND_BUTTON) {
    m_eventtype->Append(_("SYSEX Rogers Stop Change"),
...
MIDI_SEND_EVENT MIDIEventSendDialog::CopyEvent() {
  ...
  const GOMidiDeviceConfig* pInDev = m_config.m_MidiIn.FindByLogicalName(
