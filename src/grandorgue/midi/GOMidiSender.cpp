  ...
  for (unsigned i = 0; i < m_events.size(); i++) {
    ...
    if (HasStart(m_events[i].type))
      m_events[i].start = cfg.ReadInteger(
          CMBSetting, group, wxString::Format(wxT("MIDISendStart%03d"), i + 1),
          0, 0x1f, false, 0);
    if (HasLength(m_events[i].type))
      m_events[i].length = cfg.ReadInteger(
          CMBSetting, group, wxString::Format(wxT("MIDISendLength%03d"), i + 1),
          0, 0x1f, false, LengthLimit(m_events[i].type));
  }
}
...
void GOMidiSender::Save(GOConfigWriter& cfg, wxString group, GOMidiMap& map) {
  ...
  for (unsigned i = 0; i < m_events.size(); i++) {
    cfg.WriteString(group, wxString::Format(wxT("MIDISendDevice%03d"), i + 1),
                    map.GetDeviceByID(m_events[i].device));
    cfg.WriteEnum(group, wxString::Format(wxT("MIDISendEventType%03d"), i + 1),
    ...
    if (HasChannel(m_events[i].type))
      cfg.WriteInteger(group,
                       ...
                       m_events[i].channel);
    if (HasKey(m_events[i].type))
      cfg.WriteInteger(group, wxString::Format(wxT("MIDISendKey%03d"), i + 1),
                       m_events[i].key);
    ...
    if (HasStart(m_events[i].type))
      cfg.WriteInteger(group, wxString::Format(wxT("MIDISendStart%03d"), i + 1),
                       m_events[i].start);
    if (HasLength(m_events[i].type))
      cfg.WriteInteger(group,
...
bool GOMidiSender::HasChannel(midi_send_message_type type) {
  if (type == MIDI_S_NOTE || type == MIDI_S_NOTE_NO_VELOCITY ||
...
bool GOMidiSender::HasKey(midi_send_message_type type) {
  ...
  if (type == MIDI_S_NOTE || type == MIDI_S_NOTE_NO_VELOCITY ||
      ...
      type == MIDI_S_CTRL_ON || type == MIDI_S_CTRL_OFF ||
      type == MIDI_S_RPN_ON || type == MIDI_S_RPN_OFF ||
      type == MIDI_S_NRPN_ON || type == MIDI_S_NRPN_OFF ||
      type == MIDI_S_RODGERS_STOP_CHANGE || type == MIDI_S_HW_NAME_STRING ||
      type == MIDI_S_HW_NAME_LCD || type == MIDI_S_HW_STRING ||
      type == MIDI_S_HW_LCD)
    return true;
...
bool GOMidiSender::HasLowValue(midi_send_message_type type) {
  if (type == MIDI_S_NOTE_OFF || type == MIDI_S_CTRL_OFF ||
      type == MIDI_S_RPN_OFF || type == MIDI_S_NRPN_OFF ||
      type == MIDI_S_PGM_RANGE || type == MIDI_S_RPN_RANGE ||
      type == MIDI_S_NRPN_RANGE || type == MIDI_S_NOTE ||
      type == MIDI_S_NOTE_NO_VELOCITY || type == MIDI_S_RPN ||
      type == MIDI_S_NRPN || type == MIDI_S_CTRL ||
...
bool GOMidiSender::HasHighValue(midi_send_message_type type) {
  if (type == MIDI_S_NOTE_ON || type == MIDI_S_CTRL_ON ||
      type == MIDI_S_RPN_ON || type == MIDI_S_NRPN_ON ||
      type == MIDI_S_PGM_RANGE || type == MIDI_S_RPN_RANGE ||
      type == MIDI_S_NRPN_RANGE || type == MIDI_S_NOTE ||
      type == MIDI_S_NOTE_NO_VELOCITY || type == MIDI_S_RPN ||
      type == MIDI_S_NRPN || type == MIDI_S_CTRL)
...
bool GOMidiSender::HasStart(midi_send_message_type type) {
  if (type == MIDI_S_HW_NAME_STRING || type == MIDI_S_HW_NAME_LCD ||
...
unsigned GOMidiSender::KeyLimit(midi_send_message_type type) {
  ...
  if (type == MIDI_S_RPN || type == MIDI_S_NRPN || type == MIDI_S_RPN_ON ||
      type == MIDI_S_RPN_OFF || type == MIDI_S_NRPN_ON ||
...
void GOMidiSender::SetKey(unsigned key, unsigned velocity) {
  ...
  for (unsigned i = 0; i < m_events.size(); i++) {
    if (m_events[i].type == MIDI_S_NOTE) {
      ...
      e.SetKey(key);
      e.SetValue(m_events[i].low_value +
                 (velocity * (m_events[i].high_value - m_events[i].low_value)) /
                     0x7f);
      m_organfile->SendMidiMessage(e);
    }
