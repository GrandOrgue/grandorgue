...
bool GODocument::IsModified()
{
	return m_modified;
}

void GODocument::Modify(bool modified)
{
	m_modified = modified;
}
...
bool GODocument::Import(GOProgressDialog* dlg, const GOOrgan& organ, const wxString& cmb)
{
	wxBusyCursor busy;
...
void GODocument::ShowPanel(unsigned id)
{
	...
	{
		registerWindow(GODocument::PANEL, panel,
			       GOPanelView::createWithFrame(this, panel));
	}
}
...
GODefinitionFile* GODocument::GetOrganFile()
{
	return m_organfile;
}
...
void GODocument::ShowOrganDialog()
{
	...
	{
		registerWindow(GODocument::ORGAN_DIALOG, NULL,
...
void GODocument::ShowMidiList()
{
	...
	{
		registerWindow(GODocument::MIDI_LIST, NULL,
			       new MIDIList(this, NULL, m_organfile));
	}
}

void GODocument::ShowMIDIEventDialog(void* element, wxString title, GOMidiReceiverBase* event, GOMidiSender* sender, GOKeyReceiver* key, GOMidiSender* division)
{
