...
GODivisional::GODivisional(GODefinitionFile* organfile, GOCombinationDefinition& divisional_template, bool is_setter) :
...
void GODivisional::Init(GOConfigReader& cfg, wxString group, int manualNumber, int divisionalNumber, wxString name)
{
...
void GODivisional::Load(GOConfigReader& cfg, wxString group, int manualNumber, int divisionalNumber)
{
	m_DivisionalNumber = divisionalNumber;
	m_ManualNumber = manualNumber;
	...
	if (!m_IsSetter)
	{
		...
		for (unsigned i = 0; i < NumberOfStops; i++)
		{
			...
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_STOP, m_ManualNumber, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfCouplers; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_COUPLER, m_ManualNumber, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfTremulants; i++)
		{
			...
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_TREMULANT, m_ManualNumber, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
		...
		for (unsigned i = 0; i < NumberOfSwitches; i++)
		{
			...
			int s = cfg.ReadInteger(ODFSetting, m_group, buffer, -cnt, cnt, false, 0);
			pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_SWITCH, m_ManualNumber, abs(s));
			if (pos >= 0)
			{
				if (used[pos])
				{
...
{
	wxString buffer;
	const std::vector<GOCombinationDefinition::CombinationSlot>& elements = m_Template.GetCombinationElements();
...
wxString GODivisional::GetMidiType()
{
	return _("Divisional");
}
