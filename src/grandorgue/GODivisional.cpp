...
GODivisional::GODivisional(GODefinitionFile* organfile,
...
void GODivisional::Init(GOConfigReader& cfg, wxString group, int manualNumber,
...
void GODivisional::Load(GOConfigReader& cfg, wxString group, int manualNumber,
  ...
  if (!m_IsSetter) {
    ...
    unsigned NumberOfStops =
        cfg.ReadInteger(ODFSetting, m_group, wxT("NumberOfStops"), 0,
                        associatedManual->GetStopCount(), true, 0);
    ...
    unsigned NumberOfTremulants =
        cfg.ReadInteger(ODFSetting, m_group, wxT("NumberOfTremulants"), 0,
                        ...
                        m_organfile->DivisionalsStoreTremulants(), 0);
    ...
    for (unsigned i = 0; i < NumberOfStops; i++) {
      ...
      pos = m_Template.findEntry(GOCombinationDefinition::COMBINATION_STOP,
                                 m_ManualNumber, abs(s));
      if (pos >= 0) {
        if (used[pos]) {
...
void GODivisional::LoadCombination(GOConfigReader& cfg) {
  ...
  if (!m_IsSetter)
    if (cfg.ReadInteger(CMBSetting, m_group, wxT("NumberOfStops"), -1,
                        associatedManual->GetStopCount(), false, -1) == -1)
      type = ODFSetting;
  wxString buffer;
  unsigned NumberOfStops =
      cfg.ReadInteger(type, m_group, wxT("NumberOfStops"), 0,
                      associatedManual->GetStopCount(), true, 0);
  unsigned NumberOfCouplers = cfg.ReadInteger(
      type, m_group, wxT("NumberOfCouplers"), 0,
