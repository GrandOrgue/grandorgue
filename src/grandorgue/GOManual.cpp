...
GOManual::GOManual(GODefinitionFile *organfile)
    : m_group(wxT("---")), m_midi(organfile, MIDI_RECV_MANUAL),
      m_sender(organfile, MIDI_SEND_MANUAL),
      m_division(organfile, MIDI_SEND_MANUAL), m_organfile(organfile),
      m_InputCouplers(), m_KeyVelocity(0), m_RemoteVelocity(), m_Velocity(),
      m_DivisionState(), m_Velocities(), m_manual_number(0),
      m_first_accessible_logical_key_nb(0), m_nb_logical_keys(0),
      m_first_accessible_key_midi_note_nb(0), m_nb_accessible_keys(0),
      m_UnisonOff(0), m_MIDIInputNumber(0), m_tremulant_ids(0), m_switch_ids(0),
      m_name(), m_stops(0), m_couplers(0), m_divisionals(0),
      m_ODFCouplerCount(0), m_displayed(false),
...
void GOManual::Init(
  ...
  m_division.Load(
    cfg, group + wxT("Division"), m_organfile->GetSettings().GetMidiMap());
...
void GOManual::Load(GOConfigReader &cfg, wxString group, int manualNumber) {
  ...
  for (unsigned i = 0; i < nb_switches; i++) {
    ...
    unsigned new_id = cfg.ReadInteger(
      ODFSetting, group, buffer, 1, m_organfile->GetSwitchCount());
...
void GOManual::Save(GOConfigWriter &cfg) {
  ...
  m_division.Save(
    cfg, m_group + wxT("Division"), m_organfile->GetSettings().GetMidiMap());
}
