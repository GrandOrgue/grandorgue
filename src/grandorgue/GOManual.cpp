...
void GOManual::Init(GOConfigReader& cfg, wxString group, int manualNumber,
                    unsigned first_midi, unsigned keys) {
  m_organfile->RegisterSaveableObject(this);
  m_group = group;
  m_name =
      wxString::Format(_("Coupling manual %d"),
                       manualNumber - m_organfile->GetODFManualCount() + 1);
  ...
  m_division.Load(cfg, group + wxT("Division"),
                  m_organfile->GetSettings().GetMidiMap());

  SetElementID(m_organfile->GetRecorderElementID(
...
void GOManual::Load(GOConfigReader& cfg, wxString group, int manualNumber) {
  ...
  m_first_accessible_logical_key_nb = cfg.ReadInteger(
      ODFSetting, group, wxT("FirstAccessibleKeyLogicalKeyNumber"), 1,
      m_nb_logical_keys);
  ...
  m_MIDIInputNumber = cfg.ReadInteger(ODFSetting, group, wxT("MIDIInputNumber"),
                                      0, 200, false, 0);
  m_displayed =
      cfg.ReadBoolean(ODFSetting, group, wxT("Displayed"), false, false);
  unsigned nb_stops =
      cfg.ReadInteger(ODFSetting, group, wxT("NumberOfStops"), 0, 999);
  m_ODFCouplerCount = cfg.ReadInteger(ODFSetting, group,
                                      wxT("NumberOfCouplers"), 0, 999, false);
  unsigned nb_divisionals = cfg.ReadInteger(
      ODFSetting, group, wxT("NumberOfDivisionals"), 0, 999, false);
  unsigned nb_tremulants =
      cfg.ReadInteger(ODFSetting, group, wxT("NumberOfTremulants"), 0,
                      m_organfile->GetTremulantCount(), false);
  unsigned nb_switches =
      cfg.ReadInteger(ODFSetting, group, wxT("NumberOfSwitches"), 0,
                      m_organfile->GetSwitchCount(), false);
  m_manual_number = manualNumber;

  m_midi.SetIndex(manualNumber);

  for (unsigned i = 0; i < 128; i++)
    m_MidiMap[i] = cfg.ReadInteger(ODFSetting, group,
                                   wxString::Format(wxT("MIDIKey%03d"), i), 0,
                                   127, false, i);
  ...
  for (unsigned i = 0; i < nb_stops; i++) {
    m_stops.push_back(
        new GOStop(m_organfile, GetFirstLogicalKeyMIDINoteNumber()));
    buffer.Printf(wxT("Stop%03d"), i + 1);
    buffer.Printf(wxT("Stop%03d"),
                  cfg.ReadInteger(ODFSetting, group, buffer, 1, 999));
    m_organfile->MarkSectionInUse(buffer);
    m_stops[i]->Load(cfg, buffer);
    m_stops[i]->SetElementID(m_organfile->GetRecorderElementID(
        wxString::Format(wxT("M%dS%d"), m_manual_number, i)));
  }

  m_couplers.resize(0);
  for (unsigned i = 0; i < m_ODFCouplerCount; i++) {
    m_couplers.push_back(new GOCoupler(m_organfile, m_manual_number));
    buffer.Printf(wxT("Coupler%03d"), i + 1);
    buffer.Printf(wxT("Coupler%03d"),
                  cfg.ReadInteger(ODFSetting, group, buffer, 1, 999));
    m_organfile->MarkSectionInUse(buffer);
    m_couplers[i]->Load(cfg, buffer);
    m_couplers[i]->SetElementID(m_organfile->GetRecorderElementID(
  ...
  for (unsigned i = 0; i < nb_tremulants; i++) {
    ...
    unsigned new_id = cfg.ReadInteger(ODFSetting, group, buffer, 1,
                                      m_organfile->GetTremulantCount());
    for (unsigned j = 0; j < m_tremulant_ids.size(); j++)
      if (m_tremulant_ids[j] == new_id)
        throw wxString::Format(_("Manual %d: Tremulant%03d already in use"),
                               m_manual_number, new_id);
  ...
  for (unsigned i = 0; i < nb_switches; i++) {
    ...
    for (unsigned j = 0; j < m_switch_ids.size(); j++)
      if (m_switch_ids[j] == new_id)
        throw wxString::Format(_("Manual %d: Switch%03d already in use"),
                               m_manual_number, new_id);
  ...
  for (unsigned i = 0; i < nb_divisionals; i++) {
    ...
    buffer.Printf(wxT("Divisional%03d"),
                  cfg.ReadInteger(ODFSetting, group, buffer, 1, 999));
    m_organfile->MarkSectionInUse(buffer);
...
void GOManual::SetKey(unsigned note, unsigned velocity, GOCoupler* prev,
                      unsigned couplerID) {
  ...
  if (m_first_accessible_logical_key_nb <= note + 1 &&
...
void GOManual::Set(unsigned note, unsigned velocity) {
  if (note < m_first_accessible_key_midi_note_nb ||
  ...
  SetKey(note - m_first_accessible_key_midi_note_nb +
             m_first_accessible_logical_key_nb - 1,
         velocity, NULL, 0);
}
...
bool GOManual::IsKeyDown(unsigned midiNoteNumber) {
  ...
  if (midiNoteNumber - m_first_accessible_key_midi_note_nb +
      ...
      m_Velocity.size())
    return false;
  return m_Velocity[midiNoteNumber - m_first_accessible_key_midi_note_nb +
                    m_first_accessible_logical_key_nb - 1] > 0;
}
...
void GOManual::Save(GOConfigWriter& cfg) {
  ...
  m_division.Save(cfg, m_group + wxT("Division"),
                  m_organfile->GetSettings().GetMidiMap());
}
...
void GOManual::PrepareRecording() {
  ...
  for (unsigned i = 0; i < m_KeyVelocity.size(); i++)
    if (m_KeyVelocity[i] > 0)
      m_sender.SetKey(i + m_first_accessible_key_midi_note_nb,
                      m_KeyVelocity[i]);
}
...
void GOManual::ShowConfigDialog() {
  ...
  m_organfile->GetDocument()->ShowMIDIEventDialog(this, title, &m_midi,
                                                  &m_sender, NULL, &m_division);
}
