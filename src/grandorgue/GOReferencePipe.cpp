...
GOReferencePipe::GOReferencePipe(GODefinitionFile* organfile, GORank* rank,
...
void GOReferencePipe::Load(GOConfigReader& cfg, wxString group,
                           wxString prefix) {
...
void GOReferencePipe::Initialize() {
  ...
  if (strs.GetCount() != 3 || !strs[0].ToULong(&manual) ||
      !strs[1].ToULong(&stop) || !strs[2].ToULong(&pipe))
    throw(wxString) _("Invalid reference ") + m_Filename;
  if ((manual < m_organfile->GetFirstManualIndex()) ||
      (manual >= m_organfile->GetODFManualCount()) || (stop <= 0) ||
      (stop > m_organfile->GetManual(manual)->GetStopCount()) || (pipe <= 0) ||
      (pipe > m_organfile->GetManual(manual)
                  ->GetStop(stop - 1)
                  ->GetRank(0)
                  ->GetPipeCount()))
    throw(wxString) _("Invalid reference ") + m_Filename;
  m_Reference =
      m_organfile->GetManual(manual)->GetStop(stop - 1)->GetRank(0)->GetPipe(
          pipe - 1);
