...
GOMidiDeviceConfig* GOMidiDeviceConfigList::FindByPhysicalName(
  ...
  for (GOMidiDeviceConfig* pDevConf : m_list) {
    ...
    if (!candidate  // physicalName has not yet matched
...
void GOMidiDeviceConfigList::MapOutputDevice(
    const GOMidiDeviceConfig& devConfSrc,
...
void GOMidiDeviceConfigList::Load(
  ...
  unsigned count = cfg.ReadInteger(CMBSetting, m_GroupName, COUNT, 0,
                                   MAX_MIDI_DEVICES, false, 0);

  for (unsigned i = 1; i <= count; i++) {
    GOMidiDeviceConfig* const devConf = Append(
        cfg.ReadString(CMBSetting, m_GroupName, wxString::Format(DEVICE03D, i)),
        // logicalName
        cfg.ReadString(CMBSetting, m_GroupName,
                       wxString::Format(DEVICE03D_REGEX, i), false),  // regEx
        cfg.ReadBoolean(CMBSetting, m_GroupName,
    ...
    if (outputMidiDevices)  // load an input device
    {
      ...
      const wxString outDevName =
          cfg.ReadString(CMBSetting, m_GroupName,
                         wxString::Format(DEVICE03D_OUTPUT_DEVICE, i), false);
...
void GOMidiDeviceConfigList::Save(GOConfigWriter& cfg, const bool isInput) {
  ...
  for (GOMidiDeviceConfig* devConf : m_list) {
    i++;
    cfg.WriteString(m_GroupName, wxString::Format(DEVICE03D, i),
                    devConf->m_LogicalName);
    cfg.WriteString(m_GroupName, wxString::Format(DEVICE03D_REGEX, i),
                    devConf->m_RegEx);
    cfg.WriteBoolean(m_GroupName, wxString::Format(DEVICE03D_ENABLED, i),
                     devConf->m_IsEnabled);
    if (isInput) {
      cfg.WriteInteger(m_GroupName, wxString::Format(DEVICE03D_SHIFT, i),
                       devConf->m_ChannelShift);
      if (devConf->p_OutputDevice)
        cfg.WriteString(m_GroupName,
