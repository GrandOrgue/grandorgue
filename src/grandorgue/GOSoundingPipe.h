...
public:
	GOSoundingPipe(GODefinitionFile* organfile, GORank* rank, bool percussive, int sampler_group_id, unsigned midi_key_number, unsigned harmonic_number, float pitch_correction, float min_volume, float max_volume, bool retune);

	void Init(GOConfigReader& cfg, wxString group, wxString prefix, wxString filename);
