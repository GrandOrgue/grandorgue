...
// wx should be included before windows.h (from jack.h), otherwise it can not be compiled with mingw
#include <wx/log.h>
...
GOrgueSoundJackPort::~GOrgueSoundJackPort()
{
	Close();
}
...
void GOrgueSoundJackPort::JackLatencyCallback (jack_latency_callback_mode_t mode, void *data) {
	if (mode == JackPlaybackLatency) {
		GOrgueSoundJackPort * const jp  = (GOrgueSoundJackPort *) data;

		if (jp->m_Channels) {
			jack_latency_range_t range;
			
			jack_port_get_latency_range(jp->m_JackOutputPorts[0], JackPlaybackLatency, &range);
...
int GOrgueSoundJackPort::JackProcessCallback(jack_nframes_t nFrames, void *data)
{
	int rc = 0;
	GOrgueSoundJackPort * const port = (GOrgueSoundJackPort *) data;
		...
		for (unsigned int i = 0; i < nc; i ++)
		{
			jack_default_audio_sample_t *
				out = (jack_default_audio_sample_t *) 
					jack_port_get_buffer(port->m_JackOutputPorts[i], nFrames);
			
			if (port->m_IsStarted)
			{
				// copy samples from the interleaved port->m_GoBuffer to the non interleaved jack buffer
...
void GOrgueSoundJackPort::JackShutdownCallback(void *data)
{
  // GOrgueSoundJackPort * const jp = (GOrgueSoundJackPort *) data;
  
}

void GOrgueSoundJackPort::Open()
{
	Close();
	...
	m_JackClient = jack_client_open
		(CLIENT_NAME, JACK_OPTIONS, &jack_status, NULL);

	if (! m_JackClient)
	{
		if (jack_status & JackServerFailed)
		      throw wxString::Format("Unable to connect to a JACK server");
		throw wxString::Format("jack_client_open() failed, status = 0x%2.0x", jack_status);
	}
