...
void GOPiston::Load(GOConfigReader& cfg, wxString group) {
  ...
  if (type == wxT("STOP")) {
    i = cfg.ReadInteger(ODFSetting, group, wxT("ManualNumber"),
                        m_organfile->GetFirstManualIndex(),
                        m_organfile->GetODFManualCount() - 1);
    j = cfg.ReadInteger(ODFSetting, group, wxT("ObjectNumber"), 1,
  ...
  if (drawstop->IsReadOnly())
    wxLogError(_("Reversible piston connect to a read-only object: %s"),
               group.c_str());
