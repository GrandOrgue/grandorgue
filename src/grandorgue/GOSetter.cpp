...
void GOSetter::Load(GOConfigReader& cfg) {
  ...
  for (unsigned i = 0; i < FRAME_GENERALS; i++) {
    m_framegeneral.push_back(new GOFrameGeneral(
  ...
  for (unsigned i = 0; i < GENERALS * GENERAL_BANKS; i++) {
    m_general.push_back(new GOFrameGeneral(m_organfile->GetGeneralTemplate(),
                                           m_organfile, true));
  ...
  for (unsigned i = 0; i < 4 * CRESCENDO_STEPS; i++) {
    m_crescendo.push_back(new GOFrameGeneral(m_organfile->GetGeneralTemplate(),
                                             m_organfile, true));
    buffer.Printf(wxT("SetterCrescendo%d_%03d"), (i / CRESCENDO_STEPS) + 1,
                  (i % CRESCENDO_STEPS) + 1);
  ...
  m_button[ID_SETTER_CRESCENDO_PREV]->Init(cfg, wxT("SetterCrescendoPrev"),
                                           _("<"));
  m_button[ID_SETTER_CRESCENDO_NEXT]->Init(cfg, wxT("SetterCrescendoNext"),
                                           _(">"));
  ...
  m_button[ID_SETTER_TEMPERAMENT_PREV]->Init(cfg, wxT("SetterTemperamentPrev"),
                                             _("<"));
  m_button[ID_SETTER_TEMPERAMENT_NEXT]->Init(cfg, wxT("SetterTemperamentNext"),
                                             _(">"));
  m_button[ID_SETTER_TRANSPOSE_DOWN]->Init(cfg, wxT("SetterTransposeDown"),
                                           _("-"));
  ...
  m_button[ID_SETTER_GENERAL_PREV]->Init(cfg, wxT("SetterGeneralPrev"),
                                         _("Prev"));
  m_button[ID_SETTER_GENERAL_NEXT]->Init(cfg, wxT("SetterGeneralNext"),
                                         _("Next"));
}
  ...
  switch (id) {
    ...
    case ID_SETTER_TEMPERAMENT_PREV: {
      ...
      m_organfile->SetTemperament(
          m_organfile->GetSettings().GetTemperaments().GetTemperamentName(
              index));
    } break;
