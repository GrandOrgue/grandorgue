...
const struct ElementListEntry GOAudioRecorder::m_element_types[] = {
    ...
    {wxT("AudioRecorderRecordRename"), ID_AUDIO_RECORDER_RECORD_RENAME, false,
     true},
    {wxT(""), -1, false, false},
};
...
void GOAudioRecorder::Load(GOConfigReader& cfg) {
  m_button[ID_AUDIO_RECORDER_RECORD]->Init(cfg, wxT("AudioRecorderRecord"),
                                           _("REC"));
  m_button[ID_AUDIO_RECORDER_STOP]->Init(cfg, wxT("AudioRecorderStop"),
                                         _("STOP"));
...
GOEnclosure* GOAudioRecorder::GetEnclosure(const wxString& name,
                                           bool is_panel) {
  return NULL;
}
...
void GOAudioRecorder::UpdateDisplay() {
  if (!IsRecording())
    ...
    m_RecordingTime.SetContent(
        wxString::Format(_("%d:%02d:%02d"), m_RecordSeconds / 3600,
                         (m_RecordSeconds / 60) % 60, m_RecordSeconds % 60));
}
...
void GOAudioRecorder::StopRecording() {
  ...
  if (!m_DoRename) {
    ...
    GOAskRenameFile(m_Filename, m_organfile->GetSettings().AudioRecorderPath(),
                    _("WAV files (*.wav)|*.wav"));
  UpdateDisplay();
}
